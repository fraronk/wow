...
$Revision: 2690 $
$Date: 2020-09-14 00:32:07 +1000 (Mon, 14 Sep 2020) $
...
function ArkInventorySearch.Frame_Table_Refresh_Threaded( frame, thread_id )
	...
	for p, pd in ArkInventory.spairs( ArkInventory.db.player.data ) do
		...
		if pd.info.proj == WOW_PROJECT_ID then
			...
			for l, ld in pairs( pd.location ) do
				...
				if ArkInventory.ClientCheck( ArkInventory.Global.Location[l].proj ) then
	...
	if ArkInventorySearch.rebuild > 0 then
		--ArkInventory.Output2( ArkInventorySearch.rebuild, " empty entries" )
