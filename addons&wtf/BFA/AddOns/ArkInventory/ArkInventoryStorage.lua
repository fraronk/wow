...
--	ArkInventory.Output( "[EVENT_ARKINV_BANK_LEAVE_BUCKET] [", bucket, "]" )
	...
	if ArkInventory.db.option.auto.close.bank > 0 and ArkInventory.LocationIsControlled( ArkInventory.Const.Location.Bag ) then
...
function ArkInventory:EVENT_ARKINV_VAULT_LEAVE_BUCKET( )
	...
	if ArkInventory.db.option.auto.close.vault > 0 and ArkInventory.LocationIsControlled( ArkInventory.Const.Location.Bag ) then
...
--	ArkInventory.Output2( "[EVENT_ARKINV_VOID_LEAVE]" )
	...
	if ArkInventory.db.option.auto.close.void > 0 and ArkInventory.LocationIsControlled( ArkInventory.Const.Location.Bag ) then
...
function ArkInventory:EVENT_ARKINV_MAIL_LEAVE_BUCKET( )
	...
	if ArkInventory.db.option.auto.close.mail > 0 and ArkInventory.LocationIsControlled( ArkInventory.Const.Location.Bag ) then
...
function ArkInventory:EVENT_ARKINV_TRADE_LEAVE( event )
	...
	if ArkInventory.db.option.auto.close.trade > 0 and ArkInventory.LocationIsControlled( ArkInventory.Const.Location.Bag ) then
...
function ArkInventory:EVENT_ARKINV_AUCTION_LEAVE_BUCKET( )
	...
	if ArkInventory.db.option.auto.close.auction > 0 and ArkInventory.LocationIsControlled( ArkInventory.Const.Location.Bag ) then
...
function ArkInventory:EVENT_ARKINV_MERCHANT_LEAVE_BUCKET( )
	...
	if ArkInventory.db.option.auto.close.merchant > 0 and ArkInventory.LocationIsControlled( ArkInventory.Const.Location.Bag ) then
...
function ArkInventory:EVENT_ARKINV_SCRAP_LEAVE( event )
	...
	if ArkInventory.db.option.auto.close.scrap > 0 and ArkInventory.LocationIsControlled( ArkInventory.Const.Location.Bag ) then
...
--	ArkInventory.Output2( "[EVENT_ARKINV_TRANSMOG_LEAVE_BUCKET]" )
	...
	if ArkInventory.db.option.auto.close.transmog > 0 and ArkInventory.LocationIsControlled( ArkInventory.Const.Location.Bag ) then
	...
	for blizzard_id in pairs( bucket ) do
		...
		if loc_id == nil then
			...
			if not ArkInventory.Global.Mode.World then
				...
				--ArkInventory.Output( "not in world - requeue scan [", blizzard_id, "] [", loc_id, "].[", bag_id, "]" )
			...
			elseif ArkInventory.ScanStateGetRun( loc_id, bag_id ) or ArkInventory.ScanStateGetQueue( loc_id, bag_id ) then
				...
				--ArkInventory.Output( "busy - add to rescan queue [", blizzard_id, "] [", loc_id, "].[", bag_id, "]" )
...
function ArkInventory.ScanBag_Threaded( blizzard_id, loc_id, bag_id, thread_id )
	...
	local h = nil
	local quality = ArkInventory.Const.Blizzard.LE_ITEM_QUALITY_POOR
	...
	for slot_id = 1, bag.count do
		...
		if h then
			...
			quality = ArkInventory.Const.Blizzard.LE_ITEM_QUALITY_POOR
...
function ArkInventory.ScanCollectionHeirloom_Threaded( blizzard_id, loc_id, bag_id, thread_id )
	...
	for _, object in ArkInventory.Collection.Heirloom.Iterate( ) do
		...
		if object.owned then
			...
			i.count = count
			i.sb = sb
			i.q = ArkInventory.Const.Blizzard.LE_ITEM_QUALITY_HEIRLOOM
			i.item = object.item
...
function ArkInventory.ScanTradeskill( )
	...
	ArkInventory.Output( "ScanTradeskill( ) start" )
	...
	if not ArkInventory.Tradeskill.IsReady( ) then
		...
		ArkInventory:SendMessage( "EVENT_ARKINV_TRADESKILL_UPDATE_BUCKET", "NOT_READY" )
		return
	end
...
function ArkInventory.ScanTradeskill_Threaded( blizzard_id, loc_id, bag_id, thread_id )
	...
	for bag_id = 1, ArkInventory.Const.Skills.maxLearn do
	...
	ArkInventory.Output( "ScanTradeskill( ) end" )
...
function ArkInventory.ScanVoidStorage_Threaded( blizzard_id, loc_id, bag_id, thread_id )
	...
	bag.count = ArkInventory.Const.Blizzard.VOID_STORAGE_MAX
	bag.empty = 0
	bag.type = ArkInventory.BagType( blizzard_id )
	bag.status = ArkInventory.Const.Bag.Status.Active
...
function ArkInventory.ObjectCountGetRaw( search_id, thread_id )
	...
	for pid, pd in pairs( ArkInventory.db.player.data ) do
		...
		if pd.info.name and pd.info.proj == WOW_PROJECT_ID then
			...
			if not d[pid] then
				d[pid] = { location = { }, extra = { }, realm = pd.info.realm, faction = pd.info.faction, class = pd.info.class, account_id = pd.info.account_id }
				changed = true
			end
						...
						if ok then
							...
							for b in pairs( loc_data.Bags ) do
								...
								if loc_id == ArkInventory.Const.Location.Vault then
									...
									extra[loc_id][b] = ok and bc or nil
								end
