					...
					if cat_type ~= nil then
						...
						for _, cat in ArkInventory.spairs( ArkInventory.Global.Category, function(a,b) return ArkInventory.Global.Category[a].sort_order < ArkInventory.Global.Category[b].sort_order end ) do
							...
							if cat_type == t then
								...
								local cat_dis = not codex.catset.category.active[cat_z][cat_code]
								local cat_junk = codex.catset.category.junk[cat_z][cat_code]
								...
								if cat_junk then
									icon = ArkInventory.Const.Texture.Junk
								end
								...
								if abs( cat_bar ) ~= bar_id then
									ArkInventory.Lib.Dewdrop:AddLine(
										...
										"tooltipText", string.format( ArkInventory.Localise["MENU_BAR_CATEGORY_DESC"], string.format( "%s%s%s", LIGHTYELLOW_FONT_COLOR_CODE, cat.fullname, FONT_COLOR_CODE_CLOSE ), bar_id ),
										"icon", icon,
										"hasArrow", true,
									...
									ArkInventory.Lib.Dewdrop:AddLine(
										"text", n,
										"tooltipTitle", ArkInventory.Localise["CATEGORY"],
										"tooltipText", string.format( ArkInventory.Localise["MENU_BAR_CATEGORY_REMOVE_DESC"], string.format( "%s%s%s", LIGHTYELLOW_FONT_COLOR_CODE, cat.fullname, FONT_COLOR_CODE_CLOSE ), cat_bar ),
										"icon", icon,
										"hasArrow", true,
										"value", string.format( "CATEGORY_OPTION_%s", cat.id ),
										"func", function( )
			...
			if level == 3 and value then
				...
				if cat_id ~= nil then
					...
					ArkInventory.Lib.Dewdrop:AddLine(
						...
						"disabled", def_bar, -- or ( ArkInventory.Global.Options.MoveLocation == loc_id and ArkInventory.Global.Options.MoveSourceBar == cat_bar ),
						"closeWhenClicked", true,
						"func", function( )
					...
					if codex.catset.category.junk[cat_z][cat_code] then
						text = string.format( "%s: %s%s", text, GREEN_FONT_COLOR_CODE, ArkInventory.Localise["ENABLED"] )
						if cat.type_code == "RULE" or cat.type_code == "CUSTOM" then
							desc = string.format( ArkInventory.Localise["MENU_BAR_CATEGORY_JUNK_DESC"], desc, ArkInventory.Localise["DISABLE"] )
						end
					else
						text = string.format( "%s: %s%s", text, RED_FONT_COLOR_CODE, ArkInventory.Localise["DISABLED"] )
						if cat.type_code == "RULE" or cat.type_code == "CUSTOM" then
							desc = string.format( ArkInventory.Localise["MENU_BAR_CATEGORY_JUNK_DESC"], desc, ArkInventory.Localise["ENABLE"] )
						end
					end
		...
		"children", function( level, value )
			...
			if level == 1 then
				...
				if cat1 then
					...
					local cat_z, cat_code = ArkInventory.CategoryCodeSplit( cat1.id )
					local cat_dis = not codex.catset.category.active[cat_z][cat_code]
					local cat_junk = codex.catset.category.junk[cat_z][cat_code]
					...
					local cat_z, cat_code = ArkInventory.CategoryCodeSplit( cat2.id )
					local cat_dis = not codex.catset.category.active[cat_z][cat_code]
					local cat_junk = codex.catset.category.junk[cat_z][cat_code]
			...
			if level == 2 and value then
				...
				if strsub( value, 1, 16 ) == "CATEGORY_ASSIGN_" then
					...
					if k == "CUSTOM" then
						...
						ArkInventory.Lib.Dewdrop:AddLine(
							...
							"closeWhenClicked", true,
							"func", function( )
								ArkInventory.Frame_Config_Show( "settings", "category_custom" )
							end
						)
				...
				ArkInventory.MenuItemCategoryAssignOpen( offset, level, value, i, loc_id, codex, bar_id, itemname, cat0, cat1, cat2 )
	...
	if strsub( value, 1, 24 ) == "CATEGORY_CURRENT_OPTION_" then
		...
		if cat_id ~= nil then
			...
			ArkInventory.Lib.Dewdrop:AddLine(
				...
				"tooltipTitle", ArkInventory.Localise["MOVE"],
				"tooltipText", string.format( ArkInventory.Localise["MENU_BAR_CATEGORY_MOVE_START_DESC"], string.format( "%s%s%s", LIGHTYELLOW_FONT_COLOR_CODE, cat.fullname, FONT_COLOR_CODE_CLOSE ) ),
				--"disabled", cat_def, -- or ( ArkInventory.Global.Options.MoveLocation == loc_id and ArkInventory.Global.Options.MoveSourceBar == cat_bar ),
				"closeWhenClicked", true,
				"func", function( )
					ArkInventory.EditModeMove( ArkInventory.Const.Move.Category, loc_id, cat_bar, cat.id )
				end
						...
						"func", function( )
							...
							if ArkInventory.ClientCheck( ArkInventory.Const.Blizzard.Client.RETAIL ) then
								...
								if loc_id == ArkInventory.Const.Location.Bag then
									...
									if bag_id == 1 then
										SetBackpackAutosortDisabled( checked )
									else
										SetBagSlotFlag( blizzard_id, LE_BAG_FILTER_FLAG_IGNORE_CLEANUP, checked )
									end
									
								elseif loc_id == ArkInventory.Const.Location.Bank then
									
									if bag_id == 1 then
										SetBankAutosortDisabled( checked )
									elseif bag_id == ArkInventory.Global.Location[loc_id].tabReagent then
										-- already set
									else
										SetBankBagSlotFlag( blizzard_id - NUM_BAG_SLOTS, LE_BAG_FILTER_FLAG_IGNORE_CLEANUP, checked )
									end
...
function ArkInventory.MenuSwitchLocation( offset, level, value )
...
function ArkInventory.MenuSwitchLocationOpen( frame )
	...
	ArkInventory.Lib.Dewdrop:Open( frame,
		...
		"relativePoint", helper_DewdropMenuPosition( frame, true ),
		"children", function( level, value )
			ArkInventory.MenuSwitchLocation( 0, level, value )
			if level == 1 then
				ArkInventory.Lib.Dewdrop:AddLine( )
				ArkInventory.Lib.Dewdrop:AddLine(
					"text", ArkInventory.Localise["CLOSE_MENU"],
...
function ArkInventory.MenuSwitchCharacter( offset, level, value, frame )
	...
	if ( level == 1 + offset ) then
		...
		for n, tp in ArkInventory.spairs( ArkInventory.db.player.data, function( a, b ) return ( a < b ) end ) do
			...
			if tp.info.proj ~= WOW_PROJECT_ID then
				show = false
			elseif not ArkInventory.ClientCheck( tp.info.proj ) then
				show = false
			elseif tp.info.account_id ~= codex.player.data.info.account_id and not ArkInventory.Global.Location[loc_id].isAccount then
				show = false
	...
	ArkInventory.Lib.Dewdrop:Open( frame,
		...
		"children", function( level, value )
			...
			if level == 1 then
				...
				ArkInventory.Lib.Dewdrop:AddLine(
					"text", ArkInventory.Localise["MENU_LOCATION_TOGGLE"],
					"hasArrow", true,
					"value", "LOCATION"
				)
...
function ArkInventory.MenuLDBTrackingCurrencyListHeaders( offset, level, value )
	...
	if ( level == 1 + offset ) then
		...
		if not ArkInventory.LocationIsMonitored( loc_id ) then
			...
			ArkInventory.Lib.Dewdrop:AddLine(
			self:AddLine( string.format( ArkInventory.Localise["LDB_LOCATION_NOT_MONITORED"], ArkInventory.Global.Location[ArkInventory.Const.Location.Currency].Name ), 1, 0, 0 )
				"notClickable", true
			)
...
function ArkInventory.MenuLDBTrackingReputationListHeaders( offset, level, value )
	...
	if ( level == 1 + offset ) then
		...
		if not ArkInventory.LocationIsMonitored( loc_id ) then
			...
			ArkInventory.Lib.Dewdrop:AddLine(
				self:AddLine( string.format( ArkInventory.Localise["LDB_LOCATION_NOT_MONITORED"], ArkInventory.Global.Location[ArkInventory.Const.Location.Reputation].Name ), 1, 0, 0 )
				"notClickable", true
			)
		...
		"children", function( level, value )
			...
			if level == 1 then
				...
				ArkInventory.Lib.Dewdrop:AddLine(
					...
					"tooltipTitle", REAGENTBANK_DEPOSIT,
					"closeWhenClicked", true,
					"hidden", ArkInventory.ClientCheck( ArkInventory.Const.Blizzard.Client.CLASSIC ),
		...
		"children", function( level, value )
			...
			if level == 1 then
				...
				ArkInventory.Lib.Dewdrop:AddLine(
					...
					"tooltipTitle", ArkInventory.Localise["CONFIG_DESIGN_ITEM_NEW_RESET_DESC"],
					"closeWhenClicked", true,
					"func", function( )
