...
local function FindItem( loc_id, cl, cb, bp, cs, id, ct )
	...
	if loc_id == ArkInventory.Const.Location.Bank and cb == REAGENTBANK_CONTAINER and ArkInventory.db.option.restack.topup then
		-- we were restacking the reagent bank and found nothing
...
local function FindCraftingItem( loc_id, cl, cb, bp, cs )
	...
	for bag_pos, bag_id in ipairs( ArkInventory.Global.Location[loc_id].Bags ) do
		...
		if not me.player.data.option[loc_id].bag[bag_id].restack.ignore then
			...
			--ArkInventory.Output( "ignored for restack ", ArkInventory.Global.Location[loc_id].Name, ".", bag_id )
		end
...
local function StackVault( )
	...
	if not ( IsGuildLeader( ) or ( canView and canDeposit ) ) then
		ArkInventory.Output( string.format( ArkInventory.Localise["RESTACK_FAIL_ACCESS"], ArkInventory.Localise["VAULT"], bag_id ) )
		return abort, recheck
	end
...
--[[
	...
	if loc_id == ArkInventory.Const.Location.Bank then
		...
		if ArkInventory.Global.Mode.Bank then
			...
			if ArkInventory.ClientCheck( ArkInventory.Const.Blizzard.Client.RETAIL ) and ArkInventory.db.option.restack.blizzard then
...
function ArkInventory.BarTransfer( loc_id, bar_id, dst_id )
	...
	ArkInventory.Output2( "to be done - moving all items in bar ", bar_id, " to the ", ArkInventory.Global.Location[dst_id].Name )
