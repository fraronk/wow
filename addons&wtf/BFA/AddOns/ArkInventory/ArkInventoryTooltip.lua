...
ArkInventory.Const.Blizzard.TooltipFunctions = {
	...
	["SetItemKey"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetAuctionItem"] = ArkInventory.Const.Blizzard.Client.CLASSIC,
	["SetAuctionSellItem"] = ArkInventory.Const.Blizzard.Client.CLASSIC,
	["SetBagItem"] = true,
	["SetBackpackToken"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetBuybackItem"] = true,
	["SetCurrencyByID"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetCurrencyToken"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetCurrencyTokenByID"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetCraftItem"] = ArkInventory.Const.Blizzard.Client.CLASSIC,
	["SetCraftSpell"] = ArkInventory.Const.Blizzard.Client.CLASSIC,
	["SetGuildBankItem"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetHeirloomByItemID"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetHyperlink"] = true,
	["SetInboxItem"] = true,
	["SetInventoryItem"] = true,
	["SetItemByID"] = true,
	["SetLootCurrency"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetLootItem"] = true,
	["SetLootRollItem"] = true,
	["SetMerchantItem"] = true,
	["SetMerchantCostItem"] = true,
	["SetQuestCurrency"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetQuestItem"] = true,
	["SetQuestLogCurrency"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetQuestLogItem"] = true,
--	["SetQuestLogRewardSpell"] = true, -- seems pointless tracking a spell when i cant track it back to something
	["SetQuestLogSpecialItem"] = ArkInventory.Const.Blizzard.Client.RETAIL,
--	["SetQuestRewardSpell"] = true, -- seems pointless tracking a spell when i cant track it back to something
	["SetRecipeReagentItem"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetRecipeResultItem"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetSendMailItem"] = true,
	["SetToyByItemID"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetTradePlayerItem"] = true,
	["SetTradeSkillItem"] = ArkInventory.Const.Blizzard.Client.CLASSIC,
	["SetTradeTargetItem"] = true,
--	["SetUnit"] = false, --  > conflicts with OnSetUnit, do NOT use
	["SetVoidItem"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetVoidDepositItem"] = ArkInventory.Const.Blizzard.Client.RETAIL,
	["SetVoidWithdrawalItem"] = ArkInventory.Const.Blizzard.Client.RETAIL,
...
--				end
			...
			if basestat then
				...
				--ArkInventory.Output2( string.format( "%04i = %s %s", i, c, txt ) )
				if not activeonly or ( activeonly and c ~= "7f7f7f" ) then
					--ArkInventory.Output2( "A - ", ctxt )
					rv = rv .. " " .. ctxt
				end
			end
			
			--ArkInventory.Output2( "Z - ", ctxt )
	...
	for i = n, 2, -1 do
		for _, v in pairs( l ) do
			...
			if obj and obj:IsShown( ) then
				...
				if c == ArkInventory.Const.Blizzard.ALREADY_KNOWN_FONT_COLOR_CODE then
					txt = ArkInventory.TooltipCleanText( txt )
...
--	checked ok = 30936
	...
	if arg1 then
		local name, count, icon, currencyID = ArkInventory.CrossClient.GetBackpackCurrencyInfo( arg1 )
		if currencyID then
			return ArkInventory.CrossClient.GetCurrencyLink( currencyID, 0 )
		end
	end
...
--	note - this does double duty, x amount of a currency (timewarbed badges, azerite, etc)
--	note - or x amount of a reputation
...
--	checked ok = 30936
	...
	local arg1, arg2 = ... -- id, amount
	if arg1 then
		
		local rep_id = C_CurrencyInfo.GetFactionGrantedByCurrency( arg1 )
		if rep_id then
			-- currency grants reputation
			local obj = ArkInventory.Collection.Reputation.GetByID( rep_id )
			if obj then
				return obj.link
			end
		else
			-- currency
			local obj = ArkInventory.Collection.Currency.GetByID( arg1 )
			if obj then
				return obj.link
			end
		end
...
--	checked ok = 
	...
	local arg1 = ... -- id
	if arg1 then
		return ArkInventory.CrossClient.GetCurrencyLink( arg1, 1 )
	end
...
--	this is here to generically cover any hyperlinks that arent item or spell based
	...
	if osd.class == "item" or osd.class == "spell" or osd.class == "currency" or osd.class == "reputation" or osd.class == "battlepet" or osd.class == "keystone" then
		return arg1
	end
...
--	checked ok = ?
	...
	if arg1 and arg2 then
		local id = GetQuestCurrencyID( arg1, arg2 )
		local obj = ArkInventory.Collection.Currency.GetByID( id )
		if obj then
			return obj.link
		end
	end
...
--	checked ok = ?
	...
	if arg1 and arg2 then
		...
		local obj = ArkInventory.Collection.Currency.GetByID( id )
		if obj then
			ArkInventory.Output2( "SetQuestLogCurrency: ", obj.link )
			return obj.link
		end
	end
	
	--[[
	local arg1, arg2 = ... -- type, index
	if arg1 and arg2 then
		local name = ArkInventory.TooltipGetLine( tooltip, 1, true )
		local id, obj = ArkInventory.Collection.Currency.GetByName( name )
		if obj then
			return obj.link
		end
	end
...
function ArkInventory.TooltipValidateDataFromSetLootCurrency( tooltip, ... )
	...
	local arg1 = ...
	local texture, item, quantity, currencyID, quality, locked, isQuestItem, questId, isActive = GetLootSlotInfo( arg1 )
	local object = ArkInventory.Collection.Currency.GetByID( currencyID )
	if object then
		return object.link
	end
...
function ArkInventory.TooltipMyDataSave( tooltip, fn, ... )
	...
	tooltip.ARKTTD.onupdate.timer = ArkInventory.Const.Blizzard.TOOLTIP_UPDATE_TIME
		...
		if ok then
					...
					if rcd.class == ArkInventory.Const.Class.Guild then
						...
						if loc_id == ArkInventory.Const.Location.Reputation then
							...
							if rh then
								...
								location_entries[#location_entries + 1] = string.format( "%s%s", c1, txt )
							...
							if rcd.entries > 1 then
								location_entries[#location_entries + 1] = string.format( "%s%s %s%s", c1, ArkInventory.Global.Location[loc_id].Name, c2, FormatLargeNumber( ld.c ) )
							else
								location_entries[#location_entries + 1] = string.format( "%s%s", c1, ArkInventory.Global.Location[loc_id].Name )
							end
			...
			if data.class[class].player_id[pid].count > 0 then
				...
				if rh then
					...
					data.class[class].player_id[pid].t1 = string.format( "%s%s%s: %s%s", hl, c1, name, c2, FormatLargeNumber( data.class[class].player_id[pid].count ) )
					data.class[class].player_id[pid].t2 = string.format( "%s", table.concat( location_entries, ", " ) )
...
--	/run ArkInventory.TooltipDump( GameTooltip )
	...
	ArkInventory.Output2( "lines = ", c )
	for i = 1, c do
		local a, b = ArkInventory.TooltipGetLine( tooltip, i )
		ArkInventory.Output2( i, " left: ", a )
		if b ~= "" then
			ArkInventory.Output2( i, " right: ", b )
		end
	end
	
	if tooltip:GetParent( ) then
		ArkInventory.Output2( "parent = ", tooltip:GetParent( ):GetName( ) )
	else
		ArkInventory.Output2( "parent = *not set*" )
	end
	
	if tooltip:GetOwner( ) then
		ArkInventory.Output2( "owner = ", tooltip:GetOwner( ):GetName( ) )
	else
		ArkInventory.Output2( "owner = *not set*" )
	end
