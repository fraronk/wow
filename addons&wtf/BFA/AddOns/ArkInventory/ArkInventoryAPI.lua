...
--[[
	...
	called after the custom battlepet tooltip is shown, so it's behind all the normal logic checks
...
function ArkInventory.TooltipBuildBattlepet( ... )
	-- only here because BattlePet Breed ID hasnt updated and is still hooking this
end
...
function ArkInventory.API.helper_CustomBattlePetTooltipReady( tooltip, h )
	...
	if IsAddOnLoaded( "BattlePetBreedID" ) then
		-- because BattlePet Breed ID hasnt updated and is still hooking this
		ArkInventory.TooltipBuildBattlepet( tooltip, h, unpack( ArkInventory.ObjectStringDecode( h ) ) )
	end
