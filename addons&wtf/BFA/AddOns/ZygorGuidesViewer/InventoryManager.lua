...
-- #GLOBALS AdiBagsContainer1,ARKINV_Frame1,Bagnon,BagnonFrameinventory,OneBagFrame
-- #GLOBALS CloseAllBags,CloseBackpack,CloseBag,CONTAINER_OFFSET_X,CONTAINER_OFFSET_Y,CONTAINER_SCALE,ContainerFrame1,CreateFrame,DeleteCursorItem,GameTooltip,GetContainerItemEquipmentSetInfo,GetContainerItemLink,GetContainerNumSlots,GetCursorPosition,InventoryManagerFrame,InventoryMeterBarFrame,InventoryMeterFrame,LE_ITEM_CLASS_ARMOR,LE_ITEM_CLASS_WEAPON,LibRover,LibStub,NUM_BAG_SLOTS,oldCloseAllBags,OpenAllBags,OpenBackpack,OpenBag,PickupContainerItem,ReloadUI,ToggleAllBags,ToggleBackpack,ToggleBag,TooltipFrame,UIFrame,UIParent,WorldFrame,ZygorGuidesViewer,ZygorGuidesViewerFrame
...
function IM:BagFullCheck(event)
	...
	local point = {}
	point.x,point.y,point.m,point.f=LibRover:GetPlayerPosition()
...
do -- limit scope of variables used for threading
	...
	function IM:OnThreadedEvent(event)
		...
		if sleepCount >= 30 or (event and event == "ZYGORGV_STEP_CHANGED") then
			...
			local x,y,map,floor=LibRover:GetPlayerPosition()
	...
	local function Finish(returns)
		...
		local point = {}
		point.x,point.y,point.map,point.floor=LibRover:GetPlayerPosition()
		--Spoo(nil, 0, point)
		if point.x and point.y then
			local gotoCoords = {
				done=false,
