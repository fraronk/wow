...
-- #GLOBALS _SelfUpdate,haspet
-- #GLOBALS FillLocalizedClassList,GossipFrame,LE_GARRISON_TYPE_6_0,LibRover,LibStub,QuestFrame,QuestFrameGreetingPanel
-- #GLOBALS ZygorGuidesViewer_L
...
local ConditionEnv = {
	...
	_Setup = function(self)
		...
		for standing,num in pairs(ZGV.StandingNums) do self[standing:gsub("[ -]","")]=num end  -- created by Faction module

		-- build class data
		for i=1,GetNumClasses() do  local _,cl=GetClassInfo(i)  tinsert(self.classes,cl:sub(1,1):upper()..cl:sub(2):lower())  end
		
		-- Store class constants
		 local pcl = select(2,UnitClass("player")):lower()
		 for i,cl in ipairs(self.classes) do  local cl=cl:lower()  self[cl] = (pcl==cl)  end
		-- Store race constants
		 local pra = select(2,UnitRace("player")):lower()
		 for i,ra in ipairs(self.races) do  local ra=ra:lower()  self[ra] = (pra==ra)  end
		 ...
		 self['kthuman']=self['kultiran']
		 self['mgnome']=self['mechagnome']
	...
	repval = function(faction,baselevel)
		baselevel = ZGV.StandingNums[baselevel] or baselevel
	...
	completedanyq = function(...)
		local count = select("#", ...)
		for i = 1, count do
			local id = select(i, ...)
			if Parser.ConditionEnv.completedq(id) then return true end
		end
		return false
	end,
	completedallq = function(...)
		...
		for i = 1, count do
			local id = select(i, ...)
			if not Parser.ConditionEnv.completedq(id) then return false end
		end
		return true
	end,
	...
	readyanyq = function(...)
		...
		for i = 1, count do
			local id = select(i, ...)
			if Parser.ConditionEnv.readyq(id) then return true end
		end
		return false
	end,
	readyallq = function(...)
	...
	haveanyquest = function(...)
		local count = select("#", ...)
		for i = 1, count do
			local id = select(i, ...)
			if Parser.ConditionEnv.havequest(id) then return true end
		end
		return false
	end,
	haveallquest = function(...)
		local count = select("#", ...)
		for i = 1, count do
			local id = select(i, ...)
			if not Parser.ConditionEnv.havequest(id) then return false end
		end
		return true
	end,
	haveq = function(id) return Parser.ConditionEnv.havequest(id) end, -- alias
	haveanyq = function(...) return Parser.ConditionEnv.haveanyquest(...) end, -- alias
	haveallq = function(...) return Parser.ConditionEnv.haveallquest(...) end, -- alias
	...
	curmax = function(curid)
		return (select(6,GetCurrencyInfo(curid)))
	end,
	...
	questpossible = function() -- user is on quest, or has quest dialog open
		...
		if goal.quest.title then
			if GossipFrame and GossipFrame:IsVisible() then
				local DATA_IN_GOSSIP=7  -- 8.2: name, level, isTrivial, isDaily, isRepeatable, isLegendary, isIgnored
				if GetNumGossipAvailableQuests()==0 and GetNumGossipActiveQuests()==0 then return false end
				for qnum=1,GetNumGossipAvailableQuests() do
					local name,level,isTrivial = select((qnum-1)*DATA_IN_GOSSIP+1,GetGossipAvailableQuests())
					if name == goal.quest.title then
						return true
					end
				end
			elseif QuestFrameGreetingPanel and QuestFrameGreetingPanel:IsVisible() then
				...
				for qnum=1,GetNumAvailableQuests() do
					...
					if title == goal.quest.title then
						return true
					end
				end
	...
	questactive = function(id,mapid) -- mapid is optional, for backwards compatibility
		if C_TaskQuest.IsActive(id) then return true end
		if mapid then
			for i,v in pairs(C_QuestLine.GetAvailableQuestLines(mapid)) do
				if v.questID==id then return true end
			end
		end
		return false
	end,
	activepet = function(id) -- checks if pet with given id is following player in open world
		...
		local petGUID, speciesID, _, _, _, _, _, name, _, _, petID = C_PetJournal.GetPetInfoByPetID(activeGUID)
		return petID==id
	end,
...
function Parser:ParseEntry(guide,fully_parse,lastparsed)
	...
	local function assign_label_from(params)
		...
		step.label=label
		autolabel=label
		if open_stickies[label] then  -- close it
	...
	while (index<#text) do  repeat
		...
		if guide.does_macrotext_follow then -- Stop processing
			...
			if do_debug then ZGV:Debug("&parser line %d: |cffffeedd%s",linecount,line) end
			...
			for chunk in line:gmatch("%s*(.-)%s*|+") do if #chunk>0 then
				...
				if chunk=="ZGV.DevStart()" or chunk=="ZGV.DevEnd()" then 
...
local function __uncrapt(text,q)
...
tinsert(ZGV.startups,{"Parser unit tests",function(self)
	do -- unit-check ParseMapXYDist
		...
		--[[
		-- run 	Parser.ConditionEnv:_Setup() early to test this
        	local a,b,c = Parser.MakeCondition("Human Mage")  assert(a,b)
		local a,b,c = Parser.MakeCondition("ZandalariTroll Restoration Druid")  assert(a,b)
		local a,b,c = Parser.MakeCondition("Frost Mage")  assert(a,b)
		local a,b,c = Parser.MakeCondition("Human Cabbage")  assert(not a,b)
		--]]
	end
end})
