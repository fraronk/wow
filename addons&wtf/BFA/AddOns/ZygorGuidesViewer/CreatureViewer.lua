...
-- #GLOBALS ChatFrame1,ChatFrame1EditBox,CreateFrame,GameTooltip,GetMouseButtonClicked,IsShiftKeyDown,UIParent,ZygorGuidesViewer,ZygorGuidesViewer_CreatureViewer,ZygorGuidesViewer_CreatureViewer_Anchor,ZygorGuidesViewerFrameMaster
...
local NoModelFrame = CHAIN(CreateFrame("Frame", "NoModelFrame", ZygorGuidesViewer_CreatureViewer))
...
function ZGV:TryToDisplayCreature(force)
	...
	if (last_step_shown ~= self.CurrentStep) or force then
		...
		local activeStickies = ZGV:GetStickiesAt(self.CurrentStep.num,true)
...
local function SetAnchor()
	if CV.Frame.anchor then
		if not ZGV.db.profile.cvanchor then
			...
			ZGV.AssignButtonTexture(CV.Frame.anchor,ZGV.SkinDir.."titlebuttons",14,32)
		else
			viewer:SetMovable(false)
			ZGV.AssignButtonTexture(CV.Frame.anchor,ZGV.SkinDir.."titlebuttons",13,32)
		end
	end
...
function CV:UpdateSkin()
	...
	ZGV.AssignButtonTexture(CV.Frame.close,ZGV.SkinDir.."titlebuttons",6,32)
	SetAnchor()
