...
function ZGV:Options_DefineOptionTables()
	...
	do
		...
		AddOption('',{ type = 'description', name=L['opt_dispotions_title'], font=ZGV.font_dialog_gray})
			AddOption('showinlinetravel',{ type = 'toggle', width = "full", _default=false, set = function(i,v) Setter_Simple(i,v) ZGV:ShowWaypoints() ZygorGuidesViewer:UpdateFrame(true) end })
	...
	do
		...
		AddOption('travelusedhs',    { type = 'toggle', width = 200,      disabled=function() return not self.db.profile.pathfinding end, set=Setter_Travel, _default=true,  })
		AddOptionSep()
		...
		AddOption('travelusewhistle',  { type = 'toggle', width = 200,      disabled=function() return not self.db.profile.pathfinding end, set=Setter_Travel, _default=true,  })
		...
		AddOption('pathfinding_comfort',{
			type = 'select',
			values={ [1]=L["opt_pathfinding_comfort_speed"], [2]="'", [3]="'", [4]="'", [5]=L["opt_pathfinding_comfort_comfort"] },
			style = "slider",
			set = function(i,v)
				self.db.profile.pathfinding_comfort = comfort_values[v]
				update_travel()
			end,
			get = function(i,v)
				for k,v in ipairs(comfort_values) do if self.db.profile.pathfinding_comfort==v then return k end end
				return 1
			end,
			width="single",
			_default = 1
	...
	do
		...
		AddOption('autoselectitem',{ type = 'toggle', width="full", _default=false, disabled = function() return not self.db.profile.questitemselector end})
