...
function WorldQuests:SuggestWorldQuestGuide(object,questID,force,mapID)
	...
	if IsWorldQuestWatched(questID) or (object and object.isCombatAllyQuest) or (object and object.hiddenworldquest) or force then
...
local function get_quest_details(data,qid)
	...
	for factionID,factionData in pairs(faction_data) do
		if (IsQuestCriteriaForBounty(qid, factionData.quest) or (data.mapID==factionData.zone) or (data.faction==factionID)) and (not factionData.faction or factionData.faction==player_faction) then
...
function WorldQuests:GetWorldQuests()
	...
	for _,quest in pairs(quests) do
		...
		local quest_type = select(3,GetQuestTagInfo(qid)) or (quest.isCombatAllyQuest and "combatally") or (quest.isDaily and show_dailies_on_map[quest.mapID] and "daily")
