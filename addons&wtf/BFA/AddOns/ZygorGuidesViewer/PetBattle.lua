...
function PetBattle.PetJournal.JournalIconSetup()
	...
	for i=1,1000 do --Max number of pets that can be shown at once. Bliz has it set to 12 currently as well. Other addons might mess with it, so increasing. ~sinus
		...
		iconFrame.tex=iconFrame:CreateTexture("ZygorPetIconTexture"..i,"OVERLAY")
...
function PetBattle:GetPetFakeIdByLink(link,generic)
	...
	breedid,breedname = ZGV.PetBattle:GetPetBreedByLink(link)
...
function PetBattle:GetPetBreedByLink(link)
	if not link then return -1,"ERROR" end
	_,_,_,BattlePetId,BattlePetLevel,BattlePetRarity,BattlePetHP,BattlePetAtt,BattlePetSpeed,_,BattlePetName = string.find(link,"(.*)battlepet:(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(.*)%[(.*)%]")
	return PetBattle:GetPetBreedByStats(BattlePetId,BattlePetHP,BattlePetAtt,BattlePetSpeed,BattlePetRarity)
end
...
function PetBattle:GetPetBreedBySlot(bag,slot)
	...
	_,_,_,BattlePetId,BattlePetLevel,BattlePetRarity,BattlePetHP,BattlePetAtt,BattlePetSpeed,_,BattlePetName = string.find(GetContainerItemLink(bag, slot),"(.*)battlepet:(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(.*)%[(.*)%]")
	return PetBattle:GetPetBreedByStats(BattlePetId,BattlePetHP,BattlePetAtt,BattlePetSpeed,BattlePetRarity)
end

function PetBattle:GetPetBreedByStats(BattlePetId,BattlePetHP,BattlePetAtt,BattlePetSpeed,BattlePetRarity)
	BattlePetId,BattlePetHP,BattlePetAtt,BattlePetSpeed,BattlePetRarity = tonumber(BattlePetId),tonumber(BattlePetHP),tonumber(BattlePetAtt),tonumber(BattlePetSpeed),tonumber(BattlePetRarity)
	...
	for i,currentbreed in pairs(PetBattle.BreedStats) do
		calc_hp		= ceil((BaseStats[1] + currentbreed[1]) * BattlePetLevel * ((RarityMod - 0.5) * 2 + 1) * 5 + 100 - 0.5)
		calc_att	= ceil((BaseStats[2] + currentbreed[2]) * BattlePetLevel * ((RarityMod - 0.5) * 2 + 1) - 0.5)
		calc_speed	= ceil((BaseStats[3] + currentbreed[3]) * BattlePetLevel * ((RarityMod - 0.5) * 2 + 1) - 0.5)
