...
-- #GLOBALS ChatTypeInfo,CompleteQuest,GetActiveTitle,GetAvailableQuestInfo,GetAvailableTitle,GetGossipActiveQuests,GetGossipAvailableQuests,GetNumActiveQuests,GetNumAvailableQuests,GetNumGossipActiveQuests,GetNumGossipAvailableQuests,GetNumQuestChoices,GetObjectiveText,GetQuestID,GetQuestReward,GetTime,GetTitleText,IsAltKeyDown,IsQuestCompletable,QuestDetailAcceptButton_OnClick,QuestFrame,QuestFrameAcceptButton,QuestFrameDetailPanel,QuestInfoFrame,RaidBossEmoteFrame,RaidNotice_AddMessage,SelectActiveQuest,SelectAvailableQuest,SelectGossipActiveQuest,SelectGossipAvailableQuest,ZygorGuidesViewer
	...
	for qnum=1,GetNumAvailableQuests() do
		...
		if self.CurrentStep then
			for i,goal in ipairs(self.CurrentStep.goals) do
				if goal.action=="accept" and goal.quest and goal.quest.title==title and goal:GetStatus()=="incomplete" then
		...
		local isTrivial = GetAvailableQuestInfo(qnum)
...
local DATA_IN_GOSSIP=8  -- 8.3: name, level, isTrivial, isDaily, isRepeatable, isLegendary, isIgnored, questID
function ZGV:QuestAutoAccept_InGossip()
	if IsAltKeyDown() then return end
	for qnum=1,GetNumGossipAvailableQuests() do
		local name,level,isTrivial,_, _, _, _, questID = select((qnum-1)*DATA_IN_GOSSIP+1,GetGossipAvailableQuests())
		if self.CurrentStep then
			for i,goal in ipairs(self.CurrentStep.goals) do
				if goal.action=="accept" and goal.quest and goal.questid==questID and goal:GetStatus()=="incomplete" then
...
function ZGV:QUEST_COMPLETE()
	...
	-- select quest reward
	if self.db.profile.autoselectitem then
