...
function ZGV:StartupModule_Threaded(startup,timeleft)  -- resumed in _StartupThread
...
function ZGV:StartupStep()  -- called from MasterFrame
	...
	while debugprofilestop()-thisframet<self.STARTUP_INTENSITY do
		...
		self.startuptimes[ret or "?"]=(self.startuptimes[ret or "?"] or 0) + t
...
function ZGV:SetGuide(name,step,hack,silent) --hack used for testing
	...
	elseif self.CurrentGuide and self.CurrentGuide.headerdata.shared and self.db.profile.share_masterslave~=2 then 
...
function ZGV:FocusStep(num,forcefocus)
	...
	self:StopFlashAnimation()  -- possibly prevent lines staying green from previous step
...
function ZGV:PreviousStep(fast,forcefocus)
	...
	if self:ActionButtonsWillTaint() then return end
...
function ZGV:SkipStep(fast,hack,forcefocus) --Hack used for testing, forces showing endguide popup
	...
	if (not nextstep or hack) and (not self.CurrentGuide.headerdata.poiloader) --[[or (self.CurrentStep and (self.CurrentStep.num == #self.CurrentGuide.steps))--]] then
		...
		if nextguide and ZGV.db.profile.n_popup_guides and not hack then
...
function ZGV:DoUpdateFrame(full,onupdate)
					...
					if goal then
						...
						if self.db.profile.goalbackgrounds then
							...
							if (goal.action~="goto" and goal.action~="fly") and self.db.profile.goalupdateflash and needsAnimating and self.frameNeedsResizing==0 then
								...
								anim_fade.sr,anim_fade.sg,anim_fade.sb,anim_fade.sa = 1,1,1,1
								anim_fade.r,anim_fade.g,anim_fade.b,anim_fade.a = r,g,b,a
								anim_fade:Play()
							...
							elseif status=="complete" and needsAnimating and self.db.profile.goalcompletionflash and self.frameNeedsResizing==0 then
								...
								local c=ZGV.db.profile.goalbackcomplete
								anim_fade.sr,anim_fade.sg,anim_fade.sb,anim_fade.sa = 1,1,1,1
								anim_fade.r,anim_fade.g,anim_fade.b,anim_fade.a = c.r,c.g,c.b,c.a
								anim_fade:Play()
								goalsneedanimating[goal]=nil
							...
							anim_fade.r,anim_fade.g,anim_fade.b,anim_fade.a = r,g,b,a
							if anim_fade:IsDone() or not anim_fade:IsPlaying() then
								local oldr,oldg,oldb,olda = back:GetBackdropColor()
								if abs(oldr-r)>0.01 or abs(oldg-g)>0.01 or abs(oldb-b)>0.01 or abs(olda-a)>0.01 then
									...
									self:Debug("&greenlines %d not playing, changing %s%d,%d,%d|r to %s%d,%d,%d",goal.num,ZGV.ArrayToStringColor({oldr,oldg,oldb,olda}),oldr*255,oldg*255,oldb*255,ZGV.ArrayToStringColor({r,g,b,a}),r*255,g*255,b*255)
								end
							else -- still playing
	...
	if not self:ActionButtonsWillTaint() then 
...
function ZGV:UpdateFrameCurrent()
	...
	local name, line,label,icon,back,clicker,action,petaction,cooldown, lastlabel
	local height = 0
...
function ZGV:ActionButtonsAreVisible()
	for j=1,#self.stepframes do
		for i=1,LINES_PER_STEP do if self.actionbuttons[j] and self.actionbuttons[j][i] then
			if self.actionbuttons[j][i]:IsShown() then return true end
			if self.petactionbuttons[j][i]:IsShown() then return true end
			if self.actionbuttons[j][i].cd:IsShown() then return true end
		end end
	end
end

function ZGV:ActionButtonsWillTaint()
	return InCombatLockdown() and (self:ActionButtonsAreVisible() or ZGV.ActionBar.Frame:IsShown())
end
...
function ZGV:ThunderStageForceUpdate()
	...
	local GetCurrentMapAreaID,C_MapBar,SetMapByID,SetDungeonMapLevel -- bandaid for globals
...
function ZGV:UpdateFrameStepSkipping()
	...
	local enabled = self.CurrentGuide and self.CurrentStep and not self:ActionButtonsWillTaint()
...
function ZGV.InPhase(phasename)
	...
	elseif phasename=="olduldum" then
		return hasbuff("spell:316193") or hasbuff("spell:317785")
	elseif phasename=="oldvale" then
		return hasbuff("spell:317931")
...
function ZGV.MapIcon_OnUpdate(self)
