...
function Appraiser:AddItemToInventory(bag,slot)
	...
	if not itemlink then -- we did not get the link for this item yet, abort and retry
		ZGV:Debug("&gold appraiser update from AddItemToInventory "..itemid.."@"..bag..","..slot)
	...
	local BattlePetId,BattlePetName,BattlePetLevel = nil,nil,nil
...
function Appraiser:MarkActiveItem()
	...
	self.LastSellingItem = active
	Appraiser.SellManualUnselect = true
	ZGV:Debug("&gold appraiser update from MarkActiveItem")
