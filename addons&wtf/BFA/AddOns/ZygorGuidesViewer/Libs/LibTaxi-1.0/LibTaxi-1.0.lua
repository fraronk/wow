...
do
		...
		elseif event=="UNIT_FLAGS" and arg1=="player" and ZGV and ZGV.DEV and LibRover.ready then
			...
			if ontaxi then
				...
				if taxis and taxis[1] and taxis[1][2]<50 then
					...
					ZGV:Print(("DEV: Departed from |cff00ff00%s|r, counting taxi trip time."):format(Lib.traveltime_dep.name))
				end
			else
				local taxis = LibRover.TaxiWhistlePredictor:FindNearestTaxis()
				if taxis and taxis[1] and taxis[1][2]<50 and Lib.traveltime_time then
					...
					if Lib.fc_by_nodeID[arr_nodeID].neighbors[dep_nodeID]==0 then
						reversed=true
	...
	function Lib:ScanTaxiMap()
		...
		self:MarkContinentSeen(cont,current_operator)
		...
		for dest=1,NumTaxiNodes() do if TaxiNodeGetType(dest)=="REACHABLE" then
			...
			for hop=1,GetNumRoutes(dest) do
				...
				if starttag~=endtag then
					...
					fcstart.neighbors[fcend.nodeID] = fcstart.neighbors[fcend.nodeID] or 0
					...
					fcend.neighbors[fcstart.nodeID] = fcend.neighbors[fcstart.nodeID] or 0
					if new then
		...
		for c,cont in pairs(Lib.taxipoints) do repeat
			for z,zone in pairs(cont) do
				for ni,node in pairs(zone) do repeat
					...
					if not fcdata then tinsert(Lib.errors,"taxi "..node.name.." "..node.taxinodeID.." has no fcdata?") break end -- shouldn't happen, if the node has a tag...
