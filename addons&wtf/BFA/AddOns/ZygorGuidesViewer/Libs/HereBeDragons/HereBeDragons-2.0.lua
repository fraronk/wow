...
local MAJOR, MINOR = "HereBeDragons-ZGV", 11
...
if not oldversion or oldversion < 7 then
    ...
    local function processMap(id, data, parent)
        if not data then data=C_Map.GetMapInfo(id) end
...
function HereBeDragons:GetWorldCoordinatesFromZone(x, y, zone)
	if KRASARANG_IS_BUGGED and zone==418 then
		x,y=ZGV.MapCoords.FixKrasangRev(418,x,y)
	end
