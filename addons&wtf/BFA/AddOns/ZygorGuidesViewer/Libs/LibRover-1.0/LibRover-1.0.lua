...
do
	...
	if Lib then
		...
		local easterns_init = {
			...
			[1]={MAPENUM["DEATHKNELL"],MAPENUM["TIRISFALGLADES"],MAPENUM["UNDERCITY"],MAPENUM["WESTERNPLAGUELANDS"],MAPENUM["EASTERNPLAGUELANDS"],MAPENUM["SILVERPINEFOREST"],MAPENUM["HILLSBRADFOOTHILLS"],MAPENUM["ARATHIHIGHLANDS"],MAPENUM["THEHINTERLANDS"],MAPENUM["RUINSOFGILNEAS"]}, --northmost, down to Arathi (common)
			[2]={MAPENUM["ARATHIHIGHLANDS"], MAPENUM["WETLANDS"],MAPENUM["TWILIGHTHIGHLANDS"]}, --middle: arathi,wetlands,twilight
		...
		Lib.zone_is_barad = {[MAPENUM["TOLBARAD"]]=true,[MAPENUM["TOLBARADPENINSULA"]]=true}
		Lib.zone_is_vash = {[MAPENUM["SHIMMERINGEXPANSE"]]=true,[MAPENUM["KELPTHARFOREST"]]=true,[MAPENUM["ABYSSALDEPTHS"]]=true,[MAPENUM["VASHJIR"]]=true}
		...
		function Lib:UpdateConfig(profile)
			...
			Lib.cfg.use_whistle=profile.travelusewhistle
		...
		function Lib:GetCooldownWithoutGCD(what,id)
			...
			if what=="spell" then start,dur,active=GetSpellCooldown(id)
			elseif what=="item" then start,dur,active=GetItemCooldown(id)
			else return 0,0,0,0
			end
			if dur>0 then 
				local gcdtime,gcd = GetSpellCooldown(61304) -- spell created for checking global cooldowns
		...
		local function _StartupThread()
			...
			do
				...
				table.foreach(
					C_Map.GetMapChildrenInfo(946 --[[=cosmic]],Enum.UIMapType.Micro,true),
					function(i,map)
		...
		function Lib:SetupInitialQuickTravel(startnode)
			...
			for i,port in ipairs(Lib.data.portkeys) do repeat
				...
				repeat
					...
					if port.mode=="whistle" and not Lib.cfg.use_whistle then  reject_reason="use_whistle off"  break  end  -- obviously
					if port.item and not (port.mode and not_misc_item_modes[port.mode]) and not Lib.cfg.use_item_teleports then  reject_reason="use_item_teleports off"  break  end  -- don't use items other then hearthstones and whistle
				...
				local coolstart,cooldur,coolavail = self:GetCooldownWithoutGCD((port.spell and "spell") or (port.item and "item"),port.spell or port.item)
		...
		function Lib:IsDestinationImpossible(mymap,destmap)
			...
			if ZGV.GetMapContinent(destmap)==101 and level<58  then
				--Outland
				local title
		...
		local temp_fields_i = {"cost","time","mycost","mytime","status","parentlink","parent","prev","next","text","maplabel","toend","taxiFinal","taxiDestination","link","a_b","a_b__c_d","costdesc","wayp_override_icon","wayp_override_text","border_optimization","changed_modes","mode"}
			...
			for neigh,neighlink in current:IterNeighs() do
				...
				then
					...
					elseif current.type=="taxi" and current.known==false then
						...
						if mode=="fly" and current.parentlink and current.parentlink.mode~="fly" then mytime = mytime + COST_MOUNTUP end
					...
					-- If high level zones are avoided... avoid.
						if mode=="walk" and Lib.cfg.avoid_highlevel_zones then
					...
					-- Penalize uncomfortable modes of travel.
						...
						if Lib.cfg.pathfinding_comfort>0
						...
						then
							...
							if current.mode and mode~=current.mode and mode~="taxi" and current.mode~="taxi" then changed_modes=changed_modes+1 end
		...
		function Lib:ReportPath(endnode)
			...
			-- PREPARE NODES FOR DISPLAY. Assign titles based on situation.
				for n=1,#results do repeat
					...
					node.is_arrival = (a_b=="taxi_taxi" or a_b=="ship_ship" or a_b=="zeppelin_zeppelin" or a_b=="portal_portal" or a_b=="mole_misc" or text=="arrive" or a_b=="taxi_argusportal" or a_b=="taxi_ferry")
					...
					if nextnode and nextnode.type=="portal" and (nextnode.m==self.data.MapIDsByName["Abyssal Depths"][0] or nextnode.m==self.data.MapIDsByName["Shimmering Expanse"][0] or nextnode.m==self.data.MapIDsByName["Kelp'thar Forest"][0]) then  nextmap=TryBZL("Vashj'ir") end
		...
		function Lib:CheckMaxSpeeds()
			...
			local groundspeed =
				       (SP_JOURNEYMAN_RIDING and (1 + 1.0)) -- +100%
			...
			local canfly_in_draenor = canfly and playerlevel>=90 and SP_DRAENOR_PATHFINDER
			local canfly_in_legion = canfly and playerlevel>=100 and SP_BROKEN_ISLES_PATHFINDER_2 -- Broken Isles Pathfinder 2
			local canfly_in_bfa = canfly and playerlevel>=120 and SP_BFA_PATHFINDER_2 -- BfA Pathfinder 2
			...
			Lib.speeds = {
				["Azeroth"] = { groundspeed + max(BONUS_GUILDPERK_MOUNTUP), flyspeed }, -- no licences anymore, just Expert+ Riding
				["Outland"] = { groundspeed + max(BONUS_GUILDPERK_MOUNTUP), flyspeed }, --Outland (flying)
				["Northrend"] = { groundspeed + max(BONUS_GUILDPERK_MOUNTUP), flyspeed }, -- no licences anymore
				["Pandaria"] = { groundspeed + max(BONUS_GUILDPERK_MOUNTUP), flyspeed }, -- no licences anymore
			...
			for zoneid=1,2000 do
				...
				local meta = Lib.data.ZoneMeta[zoneid]
				if (system and Lib.ZoneIsOutdoor(zoneid)) or (meta.flyable==true) then
					...
					if Lib.zone_is_exo_or_belf[zoneid] or Lib.zone_is_barad[zoneid] then fly=0
					elseif Lib.zone_is_vash[zoneid] then fly=2.4  -- seahorses are fast
		...
		function Lib:IsVindicaarIn(name)  -- map ids fixed for bfa
			...
			if name=="Mac'Aree" then return map==882 or map==883 or map==884 end
		...
		function Lib:GetVindicaarPosition()  -- map ids fixed for bfa
			local map = ZGV.GetCurrentMapID()
			if (map==882 or map==883 or map==884)  then return "Mac'Aree" end
		...
		function Lib.ShowDebugMenu(parent,but)
			...
			tinsert(menu,{ text = "Show zone nodes and connections",  notCheckable=true, func=function() LibRover:ShowAllNodes() LibRover:DEV_ShowAllWalls() end })
			tinsert(menu,{ text = "Explore zone nodes",  notCheckable=true, disabled=not Spoo, func=function() LibRover:ShowAllNodes(nil,"spoo") end })
			tinsert(menu,{ text = "View Travel speed info on map",  checked=ZGV.db.profile.pointer_dev_showroverzoneinfo, func=function() ZGV.db.profile.pointer_dev_showroverzoneinfo=not ZGV.db.profile.pointer_dev_showroverzoneinfo end })
		...
		function Lib:GetPlayerPosition()
			...
			if not x then return 0,0,map end

			if map==418 then -- Krasang wilds is broken, and all coords are offset.	
				x,y = ZGV.MapCoords.FixKrasang(map,x,y)
			end
