...
function Region:Contains(node,debug)
	...
	if self.name==node.region then
		...
		local can_fly_in_zone = LibRover.maxspeedinzone[node.m][3]
		if can_fly_in_zone and not self.indoors and not self.in_flight then return false,debug and "player can fly" end
		...
		if self.indoors~=nil then
			if node.indoors~=self.indoors then return false,debug and "indoors mismatch" end
		end
