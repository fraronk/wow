...
function Node:IsTaxiKnown()
	if self.known==true then return true,"|cff00ff00known"
	elseif self.known==false then return false,"|cffff0000unknown"

	elseif (self.quest and not IsQuestFlaggedCompleted(self.quest)) then return false,"|cffff0000assumed unknown (quest incmp)"
	elseif (self.factionid and ZGV:GetReputation(self.factionid).standing<(self.factionstanding or 3)) then return false,"|cffff0000assumed unknown (faction rep)"
	elseif (self.cond_fun and not self.cond_fun()) then return false,"|cffff0000assumed unknown (cond_fun)"
	elseif (self.level and ZGV:GetPlayerPreciseLevel()<self.level) or (not C_Map.GetMapLevels(self.m) or C_Map.GetMapLevels(self.m)>ZGV:GetPlayerPreciseLevel()+5) then return false,"|cffff0000assumed unknown (high lvl)"
	else return nil,"|cffffaa00maybe?" end
end
...
function Node:tostring(withneighs)
	...
	if self.type=="taxi" then ret = ret .. (" |cff8899aa(taxi %s|cff8899aa)"):format(select(2,self:IsTaxiKnown()))  end
...
function Node:CanWalkTo(dest,debugmode)
	...
	if (n2_m==41 and n2.f==0 and (n1.y-0.8)*(n2.y-0.8)<0) then return false,debugmode and "stupid rut'theran hack" end -- Darnassus! Unwalkable to Rut'n! TEMPORARY... --TODO: make this a region.
...
function Node:CanFlyTo(dest,debug)
	...
	if flyspeed==0 then return false,debug and "player can't fly in here" end
	...
	if (dest_m==MAPENUM_DALARANDEADWIND and self.c==MAPENUM_EASTERNKINGDOMS) or (m==MAPENUM_DALARANDEADWIND and dest_c==MAPENUM_EASTERNKINGDOMS) then return true,debug and "dest is Dalaran Deadwind #30/21" end  -- LEGION temp? Dalaran is flying over Deadwind Pass, but it registers as Elwynn /21.
	if not (dest_c==self.c and dest_c>=0) then return false,debug and "not same cont" end
	...
	--[[
	if (m==504 and self.f==2)  -- Orgri or Dala  -- CAN fly in Orgri Cleft (321/2) for some time now.
	...
	if ZGV.GetMapInfo(m).mapType==4 and ZGV.GetMapInfo(dest_m).mapType==4 then return false,debug and "no shortcuts through dungeons" end
	...
	if not ZoneIsOutdoor(m1) or not ZoneIsOutdoor(m2) then return false,"No flying into/from/on indoor floors." end
