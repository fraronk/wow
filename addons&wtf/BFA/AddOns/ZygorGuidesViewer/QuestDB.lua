...
function ZGV.QuestDB:SetQuestForButton(questID,parent)
	...
	if ZGV.QuestDB:GetGuidesForQuest(questID) then
		...
		ZGV.QuestDB.questName = QuestInfoTitleHeader:GetText()
		ZGV.QuestDB.SearchIcon:Show()
	else
		ZGV.QuestDB.SearchIcon:Hide()
	end

	if parent=="QuestMapFrame" then
	...
	elseif parent=="QuestLogPopupDetailFrame" then
		ZGV.QuestDB.SearchIcon:ClearAllPoints()
