...
-- #GLOBALS AchievementFrame,AchievementFrame_LoadUI,AchievementFrame_SelectAchievement,AchievementFrame_ToggleAchievementFrame,
-- #GLOBALS C_Garrison,C_MountJournal,C_PetJournal,C_Scenario,C_TransmogCollection,C_TransmogSets
-- #GLOBALS CloseGossip,CloseQuest,CreateFrame,CreateMacro,DeleteMacro,DressUpItemLink,ERR_NOT_IN_GROUP,ERR_NOT_IN_RAID
-- #GLOBALS GetAchievementCriteriaInfo,GetAchievementInfo,GetAchievementNumCriteria,GetAvailableQuestInfo,GetCurrencyInfo,GetGossipActiveQuests,GetGossipAvailableQuests,GetInventoryItemID,GetInventorySlotInfo,GetItemCount,GetMacroIndexByName,GetMacroInfo,GetMinimapZoneText,GetMoneyString,GetNumAvailableQuests,GetNumGossipActiveQuests,GetNumGossipAvailableQuests,GetNumMacros,GetObjectiveText,GetPetActionInfo,GetQuestObjectiveInfo,GetRealZoneText,GetSpellInfo,GetSubZoneText,GetTime,GetTitleName,GetZoneText
-- #GLOBALS GossipFrame,InCombatLockdown,IsDressableItem,IsInGroup,IsInRaid,IsModifiedClick,IsMounted,IsQuestCompletable,IsShiftKeyDown,IsSpellKnown,IsTitleKnown,IsUsableSpell,LE_QUEST_FREQUENCY_DAILY,LibRover,LibTaxi,loadstring,MAX_ACCOUNT_MACROS,MAX_CHARACTER_MACROS,OpenWorldMap,PlayerHasToy,QUEST_MONSTERS_KILLED,QuestFrame,QuestFrameGreetingPanel,SendChatMessage,SetMacroItem,SetMacroSpell,UIParent,UISpecialFrames,UnitGroupRolesAssigned,UnitInVehicle,UnitOnTaxi,UnitXP,UnitXPMax,WorldMapFrame,ZygorGuidesNPCs,ZygorGuidesViewer,ZygorGuidesViewer_L
...
GOALTYPES['talknpcs'] = {
		...
		for npcrawdata in ZGV.NPCData.raw:gmatch"[^\n]+" do
			...
			for datasetindex,datasetid in pairs(self.npcids) do
				if tonumber(npcid) == tonumber(datasetid) then
					local temp = ZGV.NPCData.parseNPC(npcdata)
	...
	onenter = function(self)
		...
		local b = self.foundnpcs[1]
		if not b then return false end

		ZGV:Debug("talknpcs: Calling librover with %d points",#more_points)
...
GOALTYPES['learnpet'] = {
	...
	iscomplete = function(self)
		local species = ZGV.PetBattle.SpeciesByDisplayId[self.petid] -- pull pet species from harvested pet journal data
...
GOALTYPES['rep'] = {
	parse = function(self,params)
		...
		self.rep=ZGV.StandingNums[self.rep]  or  error("Unknown rep standing "..self.rep) 
...
GOALTYPES['fpath'] = {
	...
	iscomplete = function(self)
		return (   ZGV.db.char.taxis[self.fpathnodeid] 
			or ZGV.db.char.taxis[ZGV.LibTaxi:FindTaxi(self.fpath)] ), true
	end,
...
function Goal:OnEnter()
	...
	if self.autoscriptfun and (not ZGV.LastSkip or ZGV.LastSkip>=0) and not self.parentStep.needsreload then
		ZGV:Debug("calling autoscript %s",self.autoscript)
		ZGV.Parser.ConditionEnv:_SetLocal(self.parentStep.parentGuide,self.parentStep,self)
		self:autoscriptfun()
