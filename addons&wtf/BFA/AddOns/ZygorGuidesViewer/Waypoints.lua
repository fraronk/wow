...
function ZGV:ShowWaypoints(command)  
	...
	if not waypath then  -- improvise waypath from gotos. If this succeeds, DON'T show goto waypoints separately.
		waypath = {coords={},loop=false,ants="straight",follow="none",markers="ant_zygorange",made_from_gotos=true }  --,markers="none",follow="none",inline_travel=true
	...
	if arrowpoint and WorldMapFrame:IsShown() then 
		if (ZGV.WorldQuests.PreventMapSwitch or 0 > 0) then
