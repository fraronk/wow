...
function Pointer:Startup()
	...
	self.OverlayFrame.ZygorCoordsDEV = ZGV.ChainCall(CreateFrame("FRAME","ZygorCoordsDEV",worldMap_TargetFrame_UI)) :SetPoint("BOTTOMLEFT",10,35) :SetHeight(60) :SetWidth(600) :SetBackdrop{bgFile = "Interface/Tooltips/UI-Tooltip-Background",edgeSize = 0,tileSize=128,tile=true,insets={top=upsideup and 20 or 0,right=0,left=0,bottom=upsideup and 0 or 0}} :SetBackdropColor(0,0,0,0.6) :Hide() .__END
	self.OverlayFrame.ZygorCoordsDEV.text = ZGV.ChainCall(self.OverlayFrame.ZygorCoordsDEV:CreateFontString(nil,"ARTWORK","GameFontHighlight")) :SetAllPoints() :SetJustifyH("LEFT") .__END 
...
Pointer.Icons = {
	...
	ant_zygorange = { tex={file="mapicons",coords={0,0.5,0,0.5},r=254/255,g=97/255,b=0},   alpha=0.8, size=20, minisize=20, rotates=false, edgetex=nil, edgesize=1 },
...
function markerproto:SetIcon(icon)  -- leave icon empty to just reset the current icon
	...
	self.frame_minimap.icon:SetTexelSnappingBias(0)
	self.frame_minimap.icon:SetSnapToPixelGrid(false)
	...
	self.frame_worldmap.icon:SetTexelSnappingBias(0)
	self.frame_worldmap.icon:SetSnapToPixelGrid(false)
...
function markerproto:tostring()
	return ("%s #%d %.1f,%.1f"):format(self.m,self.m,(self.x or 0)*100,(self.y or 0)*100)
end
...
function Pointer.frame_taximap_functions.OnEvent(self,event,...)
	...
	if event == "TAXIMAP_OPENED" then
		if TaxiFrame:IsShown() then
			local texture = TaxiFrame.InsetBg:GetTexture()   
		...
		if route then  -- no path plotted, but might have been attempted.
			...
			for i=start,#points do
				...
				and not (prevnode and node.text==prevnode.text and node.border==prevnode and not ZGV.db.profile.debug_display)  then
					...
					tinsert(list,{
						text = L['pointer_arrowmenu_route_node'..(n==1 and '1' or '')]:format(n,text) .. (node.is_arrival and " (-ARRIVE-)" or ""),
						isTitle=true, notCheckable=true,
					})
					if ZGV.db.profile.debug_display then
						local str = "|cffff8800 = " .. node:tostring():gsub("(|c........%[my t)","\n%1")
						for i,text in ipairs{("\n"):split(str)} do
							tinsert(list,{ text = text, notCheckable=true, func=function() if (Spoo) then Spoo(node) end end })
						end
					end
				...
				if i==start and ZGV.db.profile.debug_display and next(LibRover.RESULTS_SKIPPED_START or {}) then
					for ni,nn in ipairs(LibRover.RESULTS_SKIPPED_START) do
						...
						for i,text in ipairs{("\n"):split(str)} do
							tinsert(list,{ text = text, isTitle=true, notCheckable=true })
						end
					end
				end
			...
			if ZGV.db.profile.debug_display then tinsert(list,{ text=("|cffff8800  Costtime: |cffffdd00%d:%04.1f"):format(floor(last.cost/60),last.cost%60), isTitle=true, notCheckable=true }) end
		end
...
function Pointer.Overlay_OnUpdate(frame,but,...)
	...
	if frame.ZygorCoordsDEV and frame.ZygorCoordsDEV:IsVisible() then
		...
		local function colorcoords(x,y)
			return ("|cffffffff%s|cffcccccc.%s|cff888888,|cffffffff%s|cffcccccc.%s"):format(("%.2f,%.2f"):format(x or -1,y or -1):match("([0-9-]+)%.(%d+),([0-9-]+)%.(%d+)"))
		end
		local s=("|cffccccccCursor: |cffffffff%s|cff888888##|cffffffff%d |cffaaaaaa(%s/%d)|r %s |cffaaaaaa(map type: %s%s)\n"):format(
			ZGV.GetMapNameByID(mm),mm,
			mmf and mmf[1] or ZGV.GetMapNameByID(mm),mmf and mmf[2] or 0,
			colorcoords(mx*100,my*100),
			maptype[ZGV.GetMapInfo(mm).mapType] or "type "..ZGV.GetMapInfo(mm).mapType,
			(LibRover.data.ZoneMeta[mm] and LibRover.data.ZoneMeta[mm].hostile and ", hostile" or ""))
		s = s .. ("|cffccccccPlayer: |cffffffff%s|cff888888##|cffffffff%d |cffaaaaaa(%s/%d)|r %s%s |cffaaaaaa(raw: %d,%d @%d)\n"):format(
			ZGV.GetMapNameByID(pm),pm,
			pmf and pmf[1] or ZGV.GetMapNameByID(pm),pmf and pmf[2] or 0,
			colorcoords(px*100,py*100),
			remapfrom and (" |cffff8888(remap from %d #%d)"):format(remapfrom,remapnum) or "",
			rawx,rawy,rawi)
		if mm~=pm and tx>0 and ty>0 then s = s .. ("|cffddddddPlayer as on |cffffffff%s|cff888888##|cffffffff%d|cffdddddd: %s\n"):format(
			ZGV.GetMapNameByID(mm),mm,
			colorcoords(tx,ty)
			) end
		s = s .. ("|cffccccccDistance: %.2f yd"):format(HBD:GetZoneDistance(pm,px,py, mm,mx,my) or 0)
...
function Pointer:DoCorpseCheck(event)
	if self:IsCorpseArrowNeeded() then
		if ZGV.CurrentStep and ZGV.CurrentStep.zombiewalk then
			...
			-- corpse arrow
			if not self.corpsearrow_shown then 
