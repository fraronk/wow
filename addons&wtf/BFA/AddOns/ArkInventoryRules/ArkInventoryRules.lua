...
$Revision: 2692 $
$Date: 2020-09-14 10:49:36 +1000 (Mon, 14 Sep 2020) $
...
function ArkInventoryRules.AppliesToItem( i )
	...
	for cat_code in ArkInventory.spairs( r, function(a,b) return ( r[a].order or 9999 ) < ( r[b].order or 9999 ) end ) do
...
function ArkInventoryRules.System.boolean_expansion( ... )
	...
	if ac == 0 then
		...
		if LE_EXPANSION_LEVEL_CURRENT == ArkInventoryRules.Object.info.expansion then
			return true
		end
...
function ArkInventoryRules.System.boolean_outfit_itemrack( ... )
	...
	if not ArkInventoryRules.Object.info.osd.h then
		return false
	end
	...
	if ArkInventory.ClientCheck( ArkInventory.Const.Blizzard.Client.CLASSIC ) then
		-- only use the first 7 values in classic
		pattern1 = "^item:(%d-:%d-:%d-:%d-:%d-:%d-:%d-:)"
		pattern2 = "^(%d-:%d-:%d-:%d-:%d-:%d-:%d-:)"
	end
	
	local id = string.match( ArkInventoryRules.Object.info.osd.h or "", pattern1 ) or 0
	
	local Outfits = { }
	
	for setname, set in pairs( ItemRackUser.Sets ) do
		if string.sub( setname, 1, 1 ) ~= "~" then
			--ArkInventory.Output( "setname=[", setname, "]" )
			--ArkInventory.Output( "looking for [", id, "]" )
			for k, setitem in pairs( set.equip ) do
				--ArkInventory.Output2( "pos=[", k, "], item=[", setitem, "], id=[", id, "]" )
				if not ( setitem == 0 or setitem == nil ) then
					local setitem = string.match( setitem, pattern2 ) or 0
					--ArkInventory.Output2( "slot[",k,"]=[", setitem, "]" )
					if id == setitem then
						table.insert( Outfits, string.trim( setname ) )
						--ArkInventory.Output( "added set [", setname, "] for item [", ArkInventoryRules.Object.info.osd.h, "]" )
						break
					end
				end
			end
