...
function ZGV:OnEnable()
	...
	self:AddEvent("NEW_WMO_CHUNK")
	
	self:AddEvent("MAP_BAR_UPDATE")
	...
	self:AddEvent("UPDATE_VEHICLE_ACTIONBAR") -- refresh actionbar when vehicle/pet is summoned/dismissed

	self:AddMessage("ZYGORGV_LOADING_TOPLEVEL_GROUPS_UPDATED")

	if self.db.profile.poketarget then
		self:AddEvent("UPDATE_MOUSEOVER_UNIT")
		self:AddEvent("CURSOR_UPDATE")
	end
		

	self:Hook_QuestChoice()

	if TaskPOI_OnClick then hooksecurefunc("TaskPOI_OnClick", function(frame,button) ZGV:SuggestWorldQuestGuide(frame) end) end
	if WorldMapPOI_OnClick then hooksecurefunc("WorldMapPOI_OnClick", function(frame,button) ZGV:SuggestGuideFromBlizzardIcon(frame) end) end
...
local function _StartupThread()
	...
	-- TODO remove this line. Blizzard calls a secure function in UpdateMicroButton that is called by functions we use. Don't need to update the micro buttons on world map open so overwrite it.
	-- Introduced in 5.4.1, will likely be patched on Blizzard's end but until then.
...
function ZGV:AddMessage(event,func)
	tinsert(self.Events[event],func or true)
	if #self.Events[event]==1 then self:RegisterMessage(event,"EventHandler") end
end

function ZGV:RemoveHandler(event,removefunc)
    for num,func in ipairs(self.Events[event]) do
...
function ZGV:LoadInitialGuide(fastload)
	...
	self:QuestTracking_CacheQuestLog()  -- just in case it didn't get cached before.
...
function ZGV:SetGuide(name,step,hack) --hack used for testing
	...
	if guide then
		...
		if self.CurrentGuide and not was_poi and self.CurrentGuide~=guide then self.CurrentGuide:Unload() end
		...
		if guide.steps then
			...
			if stepobj then
				...
				self:Print(L["message_loadedguide"]:format(name,step),false,"FORCE")
...
function ZGV:FocusStepUnquiet()
	...
	--[[
	-- Pois don't get waypoints now, as of 2015-06-18. They're either "focused steps" or not.
	local pois = self.Poi:GetListOfPois()
	for _,poi in ipairs(pois) do
		if poi.is_expanded then self:ShowWaypoints("sticky",poi) end
	end
	--]]
...
function ZGV:SkipStep(fast,hack,forcefocus) --Hack used for testing, forces showing endguide popup
	...
	if not nextstep and not hack then  -- when not forced, that is: usually.
		...
		if fast and self.db.profile.instantskip then nextstep2,stepnum,guide=self.CurrentStep:GetNextCompletableStep()
		else nextstep2,stepnum,guide=self.CurrentStep:GetNextValidStep() end  -- always returns a step, unless we're at the end.
	...
	if not nextstep or hack --[[or (self.CurrentStep and (self.CurrentStep.num == #self.CurrentGuide.steps))--]] then
		-- final step
		self.pause = true
		self.fastforward = false
...
function ZGV:TryToCompleteStep(force)
	...
	local _,poi
	if self.db.profile.poienabled and ZGV.Poi and ZGV.Poi.CurrentPoiSteps then
		for _,poi in ipairs(ZGV.Poi.CurrentPoiSteps) do
			poi:CheckVisitedGotos()
		end
	end
...
function ZGV:UpdateFrameAddStep(step,mode)
	...
	self.stepframes[ZGV.framenum].is_poi = (mode == "poi")
...
function ZGV:UpdateFrame(full,onupdate)
	...
	if self.CurrentGuide and self.CurrentGuide.steps and self.CurrentGuide.fully_parsed then
		...
		local pois
		if self.db.profile.poienabled then
			pois = self.Poi:GetListOfPois()
		end
		...
		-- FIRST, assign steps to frames.
			...
			-- Now the stickies that accompany it, if any.
				...
				if pois then
					for _,poi in ipairs(pois) do
						if not iscomplete then  ZGV:UpdateFrameAddStep(poi,"poi")  end
					end
				end
		...
		for stepframenum = 1,self.StepLimit do repeat
			...
			local untruncated_text_width = frame:GetWidth()-icon_indent-2*SkinData("StepLineMarginX")
			for l=1,LINES_PER_STEP do
			...
			elseif frame.is_poi then
				frame:SetBackdropBorderColor(pr,pg,pb,1)
			else
					...
					if goal then
						...
						if self.db.profile.goalbackgrounds then
							...
							elseif status=="complete" and justCompleted and self.db.profile.goalcompletionflash and self.frameNeedsResizing==0 then
								...
								-- self.completionelapsed = 0  -- experimental delay
							elseif goal.parentStep and goal.parentStep.PoiStep and goal.parentStep.PoiStep.flashGoals then
			...
			if poigoal and poigoal.parentStep and poigoal.parentStep.PoiStep and poigoal.parentStep.PoiStep.flashGoals then 
				poigoal.parentStep.PoiStep.flashGoals = false
			end

			-- Highlight for multi-waypoint cycling
			-- Please tell me if this is in the wrong place . . . ~~ Jeremiah
			...
			if frame.lines[ZGV.CurrentStep.current_waypoint_goal]
			-- and ZGV.CurrentStep.numberMappedGoals and ZGV.CurrentStep.numberMappedGoals > 1
			then
				currentBackdropColor = {frame.lines[ZGV.CurrentStep.current_waypoint_goal].back:GetBackdropColor()}
			...
			if showallsteps then
				...
				if stepdata==focusedstep or ZGV.CurrentStep==focusedstep and frame.is_sticky then -- focused step or it's sticky
					if stepframenum==1 or frame.is_sticky then
						frame:SetAlpha(1.0)
					elseif stepframenum==1 or frame.is_poi then
						frame:SetAlpha(1.0)
					else
...
function ZGV:MatchProfs(fitprof,levelmin,levelmax)
	local data = self:GetSkill(fitprof)
...
function ZGV:FindEvent(eventName)
	...
	local calmonth,calyear=CalendarGetMonth()
	...
	-- UPDATE: OpenCalendar and UI loading are now done during startup. This queries for events in advance.

	if month~=calmonth or year~=calyear then
		-- reset to current month
		if CalendarFrame and CalendarFrame:IsVisible() then  return false  end  --They are looking at the calendar, too bad

		CalendarSetAbsMonth(month,year)
		
		calmonth,calyear = CalendarGetMonth()
...
tinsert(ZGV.startups,{"Preiniting Calendar",function(self)
	if not CalendarFrame then Calendar_LoadUI() OpenCalendar() end --Makes the UI available, and queries server for events.
end})
...
function ZGV:SetStepFocus(step)
	...
	if ZGV.Poi.Guides and #ZGV.Poi.Guides>0 then
		for j,g in pairs(ZGV.Poi.Guides) do
			for i,v in pairs(g.steps) do
				v.isFocused = false
			end
		end
	end
	
	if ZGV.Poi.ActivePoiStepSet then 
		ZGV.Poi.ActivePoiStep.isFocused = false	
	end
	...
	if step.is_poi then
		ZGV.Poi.ActivePoiStepSet=true
		ZGV.Poi.ActivePoiStep=step
		ZGV.Poi.ActivePoiStepNum=step.ident
	end
	
	self:Debug("Focus changed to step %s (%s)",step.num, step.parentGuide==ZGV.Poi.Guide and "poi" or "guide")
...
function ZGV:GetFocusedStep()
	if ZGV.Poi.ActivePoiStepSet and ZGV.Poi.ActivePoiStep.isFocused then
		return ZGV.Poi.ActivePoiStep
	elseif ZGV.CurrentStep and ZGV.CurrentStep.isFocused then
...
function ZGV:GoalOnClick(goalframe,button)
	...
	if self.db.profile.showcountsteps>0 and stepframe.step~=self.CurrentStep and not stepframe.is_sticky and not stepframe.is_poi then return end -- no clicking on non-current steps in compact mode
...
function ZGV:OpenQuickStepMenu(stepframe,goalframe)
	...
	if not goal.parentStep.is_poi and not ZGV.db.char.ActivatedPois[goal.parentStep.ident] then
		tinsert(menu,{
	...
	if goal.parentStep.is_poi and ZGV.db.char.ActivatedPois[goal.parentStep.ident] then
		tinsert(menu,{
			text = "Remove this point of interest",
			tooltipOnButton = false,
			func = function()  
				ZGV.db.char.ActivatedPois[goal.parentStep.ident]=false
				ZGV.Poi:RefreshMapIcons()
	...
	for i=1,#path do
		...
		if not found then
			...
			if i==1 then  -- we're at top level
				...
				self:SendMessage("ZYGORGV_LOADING_TOPLEVEL_GROUPS_UPDATED")
			end
			group=gr
		end
	end
...
function ZGV:Debug (msg,...)
	...
	if true then -- self and self.db and self.db.profile and self.db.profile.debug and not self.db.profile.quiet then
		...
		if func then
			...
			if self.db.profile.debug_fps then
				chatframe:AddMessage(("|cffffee77Z|r: %s%06.03f+%03d|r@%03d |cff00ddbb#%d: %s%s"):format(timecolor,(t-self.loadtimeGT),debugms,GetFramerate(),self.DebugI,debugcolor,formatted_msg))
			else
...
function ZGV:FindGuides(sub)
	...
	local function do_find(sub,scope,into)
		for gi,g in pairs(scope) do
			if not (g.poi or g.type=="TEST") and not (logic=="OR" and already_found[g]) then
...
ZGV.Quest_Choices = {}
function ZGV:ZYGOR__QUEST_CHOICE_SENT(event,id,choice)
...
function ZGV:FindWorldQuestStep(questID)
	local labelstep
...
function ZGV:SuggestWorldQuestGuide(object,questID,force)
	...
	if IsWorldQuestWatched(questID) or force then
		local labelstep = self:FindWorldQuestStep(questID)
...
function ZGV:SuggestGuideFromBlizzardIcon(object)
	...
	local guidetitles = {
		["Dailies Guides\\Legion\\World Quests"] = {content="rare elite", prefix="rare"},
