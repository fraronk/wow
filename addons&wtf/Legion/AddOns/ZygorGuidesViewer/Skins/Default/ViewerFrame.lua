...
function ZygorGuidesViewerFrame_FindNearest_UpdateVisibility()
	if FindNearestFrame:IsMouseOver() then 
		hide_timer_main = 0
	else
...
function ZygorGuidesViewerFrame_SearchButton_OnClick(self,button)
	...
	if not self.FindNearestFrame then
		-- Create mini-frame for showing options
		self.FindNearestFrame = CHAIN(CreateFrame("Frame","FindNearestFrame",self))
			...
			:SetScript("OnUpdate", ZygorGuidesViewerFrame_FindNearest_UpdateVisibility)
		.__END
		self.FindNearestFrameTitle = CHAIN(self.FindNearestFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal"))
			:SetPoint("TOPLEFT", self.FindNearestFrame, "TOPLEFT", titleLeftAdjust, -titleTopAdjust)
		...
		self.FindNearestFrameClass = CHAIN(CreateFrame("Frame","FindNearestFrameClass",self))
			...
			:SetFrameStrata("HIGH")
			:RegisterEvent("CURSOR_UPDATE")
			--:SetScript("OnEvent", function(self) self:Hide() end)
		.__END
		self.FindNearestFrameClassTitle = CHAIN(self.FindNearestFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal"))
			:SetPoint("TOPLEFT", self.FindNearestFrameClass, "TOPLEFT", titleLeftAdjust, -titleTopAdjust)
			:SetTextColor(1, 1, 1)
			:SetFont(ZGV.FontBold,titleTextSize)
		.__END
		--]]
		...
		self.FindNearestFrameProff = CHAIN(CreateFrame("Frame","FindNearestFrameProff",self))
			...
			--:SetScript("OnEvent", function(self) self:Hide() end)
		.__END
		self.FindNearestFrameProffTitle = CHAIN(self.FindNearestFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal"))
			:SetPoint("TOPLEFT", self.FindNearestFrameProff, "TOPLEFT", titleLeftAdjust, -titleTopAdjust)
			:SetTextColor(1, 1, 1)
			:SetFont(ZGV.FontBold,titleTextSize)
		.__END
	...
	--if ZGV.db.profile.im_prefer_repair then
...
function ZygorGuidesViewerFrame_SearchButton_OnEnter(self)
	GameTooltip:SetOwner(ZGV.Frame, "ANCHOR_TOP")
