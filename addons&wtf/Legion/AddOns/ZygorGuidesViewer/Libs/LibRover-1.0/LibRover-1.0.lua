	...
	if Lib then
		...
		function Lib:ProcessHalfbakedNeighbourCache()
			...
			for i,s in ipairs(cache) do
				...
				if debugprofilestop()-t1>STARTUP_INTENSITY and coroutine.running() then coroutine.yield("process halfbaked cache") t1=debugprofilestop() end
			end
		...
		function Lib:IsDestinationImpossible(mymap,myfloor,destmap,destfloor)
			...
			elseif (destmap==606 or destmap==683) and not canfly then --Mount Hyjal
		...
		function Lib:FindPath(am,af,ax,ay,bm,bf,bx,by, handler, extradata, force_new, quiet)
			...
			af = af and ZGV:SanitizeMapFloor(am,af,"is_current")
			...
			self:Debug("&_PUSH |cff00ff88FindPath|r:  |cffaaff88%s|r##%d/%d |cffffffff%.1f|r,|cffffffff%.1f|r --> |cffaaff88%s|r##%d/%d |cffffffff%.1f|r,|cffffffff%.1f|r",
				am and GetMapNameByID(am) or am or "nil",am or 0,af or 0,ax and ax*100 or -1,ay and ay*100 or -1,
				bm and GetMapNameByID(bm) or bm or "nil",bm or 0,bf or 0,bx and bx*100 or -1,by and by*100 or -1)


			if not am or not af or not bm or not bf or am<=0 or bm<=0 or not ax or not ay or (ax==0 and ay==0) then  
				self:Debug("&_POP |cff00ff88FindPath|r |cffff0000failed|r: no start or end location (%s/%s %s,%s to %s/%s %s,%s). Force update? %s",am,af,ax,ay,bm,bf,bx,by,self.force_update_counter)
				...
				if extradata.retries>0 then
					...
					self:QueueFindPath(0,0,0,0,bm,bf,bx,by, handler, extradata, force_new, quiet)
				else
		...
		local temp_fields = {cost=1,time=1,mycost=1,mytime=1,status=1,parentlink=1,parent=1,prev=1,['next']=1,text=1,maplabel=1,toend=1,taxiFinal=1, link=1,a_b=1,a_b__c_d=1,costdesc=1,wayp_override_icon=1,wayp_override_text=1,border_optimization=1}
		local temp_fields_i = {}
				...
				then
					...
					-- Penalize not-debug-forced path. Experimental...
						if lib_debug_forcepath and (lib_debug_forcepath[neigh] or 99)-(lib_debug_forcepath[current] or 99)~=1 then
							...
							if cost_debugging then costdesc = costdesc .. "not in forcepath; " end
						end
					-- ==
					...
					if debugged_pair then
						print(("|cffffffffLinking %d to %d: mode %s, cost %.1f, time %.1f, '%s' (total time %.1f, cost %.1f)"):format(
							current.num, neigh.num, neighlink.mode, mytime,mycost, costdesc, time,cost))
					end
		...
		function Lib:ReportPath(endnode)
			...
			-- PREPARE NODES FOR DISPLAY. Assign titles based on situation.
				for n=1,#results do repeat
					...
					if (a_b=="taxi_taxi" or a_b=="ship_ship" or a_b=="zeppelin_zeppelin" or a_b=="portal_portal" or text=="arrive" or a_b=="taxi_argusportal")
			...
			elseif self.updating and not self.delayed_by_taxi and not InCombatLockdown() and lbm and not ZGV.db.profile.disable_travel_updates then
				...
				if self.force_update_now then
					...
					if not x or not y then
						self:Debug("Force update: current location is unknown (%s %s %s %s). May retry: %d/50.",tostring(x),tostring(y),tostring(m),tostring(f),self.force_update_counter)
						return
		...
		function Lib:CheckMaxSpeeds()
			...
			for _,zoneid in ipairs(area_maps_table) do
				...
				if system then
					...
					elseif system==1184 then speed=Lib.speeds["Argus"]
					else speed=1.0 end
		...
		--[[
		function Lib:RecordVindicaar()
			local z1,z2,am,af = HBD:GetPlayerZonePosition(true)
			if am==1170 then
				ZGV.db.char.Vindicaar = "Mac'Aree"
			elseif am==1171 then
				ZGV.db.char.Vindicaar = "Antoran Wastes"
			else
				-- leaving argus maps resets Vindicaar to Krokuun
				ZGV.db.char.Vindicaar = "Krokuun"
			end
		end
		--]]

		function Lib:IsVindicaarIn(name)
			if name=="Mac'Aree" then return GetCurrentMapAreaID()==1170 end
			if name=="Antoran Wastes" then return GetCurrentMapAreaID()==1171 end
			if name=="Krokuun" then return (GetCurrentMapAreaID()~=1170 and GetCurrentMapAreaID()~=1171) end
		end

		function Lib:GetVindicaarPosition()
			if GetCurrentMapAreaID()==1170  then return "Mac'Aree" end
			if GetCurrentMapAreaID()==1171  then return "Antoran Wastes" end
			return "Krokuun"
		end
		...
		function LibRover:DebugNodes(n1,n2,...)
			...
			if n1 and n2 then
				...
				local function debugit(n1,n2)
					...
					local walk,fly,walkwhy,flywhy,mode = n1:DoLinkage(n2,true)
					print(("Testing linkage:  can fly? %s,%s;  can walk? %s,%s;  mode=%s"):format(yesno(fly),flywhy or "-",yesno(walk),walkwhy or "-",mode or "-")) --dry run
					local meta
