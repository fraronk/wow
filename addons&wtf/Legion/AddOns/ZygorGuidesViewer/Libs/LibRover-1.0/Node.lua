...
function Node:DoLinkage(n2,dryrun)
	...
	if canfly then
		...
		canwalk,reasonwalk = n1:CanWalkTo(n2,"debug")
		if canwalk then
...
function Node:GetText(prevnode,nextnode,dir)
	...
	elseif self.type=="garrison_inn" then
		...
		return ("%s %d,%d"):format(MapName(self),(self.x or 0)*100,(self.y or 0)*100)
	end
end
...
function Node:CanWalkTo(dest,debugmode)
	...
	if (debugmode) then
		...
		if n1.region==n2.region then reason=reason.."same region; " else reason=reason.."diff region; " end
	...
	if Lib.greenborders:CanCross(n1.m,n2.m) then return true,reason end
...
function Node:CanFlyTo(dest)
	...
	if not flyspeed then return false,"player can't fly in "..m end
	...
	if self.type=="border" and not self.border_in_flight then return false,"src is a ground border so ignored" end
	if dest.type=="border" and not dest.border_in_flight then return false,"dest is a ground border so ignored" end  -- this reason is used verbatim in DoLinkage, update it there!
	...
	if m==928 or m==933 or dest_m==928 or dest_m==933 then return false,"no flying in Isle of Thunder" end
	if m==929 then return false,"no flying out of Isle of Giants" end --Can fly into this area, but not fly out of it, or around it
	if m==951 then return false,"no flying out of Timeless Isle" end --Can fly into this area, but not fly out of it, or around it
	if m==499 or dest_m==499 then return false,"Isle of Quel'Danas no flying there!" end
	if (m==382 and dest_m~=382) or (m~=382 and dest_m==382) then return false,"'no flying' in Undercity" end
	if self.region~=dest.region and self.regionobj and self.regionobj.in_flight then return false,"src in diff in_flight region" end
	if self.region~=dest.region and dest.regionobj and dest.regionobj.in_flight then return false,"dest in diff in_flight region" end
