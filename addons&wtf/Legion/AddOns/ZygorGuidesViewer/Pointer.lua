...
function Pointer:Startup()
	...
	if ZGV.DEV then
		...
		:Hide()
		.__END
		self.OverlayFrame.PathDebugButton = ZGV.ChainCall(CreateFrame("BUTTON","ZGVPointerDebugButton",WorldMapFrame.UIElementsFrame,"UIPanelButtonTemplate"))
...
function Pointer:Debug_AddPointToPath(m,f,x,y)
	self:Debug_MakeWaypath()
	local coords = ZGV.CurrentStep.waypath.coords
	...
	if before then
		tinsert(ZGV.CurrentStep.waypath.coords,before,{map=m,floor=f,x=x,y=y})
		ZGV:ShowWaypoints()
	end
end
...
function Pointer:Debug_DumpPath()
	...
	local s=""
	s="\tmap ".. self.GetMapNameByID2(waypath.coords[1].map) .."/"..waypath.coords[1].floor.."\n"
	s=s.."\tpath    "
	...
	for i,coord in ipairs(waypath.coords) do
		...
		local maybemap=""
		if i>1 and (waypath.coords[i-1].map~=coord.map or waypath.coords[i-1].floor~=coord.floor) then maybemap=self.GetMapNameByID2(coord.map).."/".. coord.floor end
...
--		end
		...
		repeat  -- left click handling
			...
			if Pointer.debug_patheditmode then
				Pointer:Debug_AddPointToPath(GetCurrentMapAreaID(),GetCurrentMapDungeonLevel(),x,y)
				ZGV:ShowWaypoints()
				return
			end
...
function Pointer:GetNextInPath(pathname,testway)
	...
	elseif pathfollow == "smart" then
		...
		else px,py,pm,pf=HBD:GetPlayerZonePosition(true) end
		...
		for i=1,#pathpoints do
			...
			for s=0,subdivs-1 do
				...
				local dist = HBD:GetZoneDistance(pm,pf,px,py,way.m,way.f,x,y) or 9999
...
function Pointer:ShowSet(waypath,name)
	...
	if waypath and waypath.coords and #waypath.coords>1 then  -- show ants, or just the path, anyway.
		...
		for k,point in ipairs(points) do
			...
			if point.player or marker=="none" or (point.pathnode and point.pathnode.a_b__c_d) == "taxi_taxi__taxi_taxi" or not ZGV.db.profile.mapicons then
