...
local IM = {
	...
	defaultRight = CONTAINER_OFFSET_X / CONTAINER_SCALE,
	defaultBottom = CONTAINER_OFFSET_Y / CONTAINER_SCALE,
	...
	displayWidth = defaultWidth, -- current width
...
function IM:CreateFrames()
	...
	--IM.InventoryManagerFrame--
		IM.InventoryManagerFrame = CHAIN(CreateFrame("Frame","InventoryManagerFrame",UIParent))
			:SetSize(IM.displayWidth,IM.displayHeight)
			--:SetPoint(db.im_lastPosition)
			:SetBackdrop(IM.SkinData("Backdrop"))
			:SetBackdropColor(unpack(IM.SkinData("BackdropColor")))
			:SetBackdropBorderColor(unpack(IM.SkinData("BackdropBorderColor")))
			:SetFrameStrata(IM.strata)
			:SetFrameLevel(1)
			:SetClampedToScreen(true)
			:SetMovable(true) :SetResizable() :EnableMouse(true) :RegisterForDrag("LeftButton")
			:Show()
		.__END
		...
		function IM.InventoryManagerFrame:OnDragStop()
			...
			-- 2nd index is specific to addon and could take a lot
			-- of space in the saved data file.
			db.im_lastPosition[2] = nil
		...
		function IM.InventoryManagerFrame:OnUpdate()
			...
			if IM.isDragging then
				...
				IM.InventoryManagerFrame:SetPoint("BOTTOMLEFT",UIFrame,"BOTTOMLEFT",IM.mouseX - IM.dragDeltaX,IM.mouseY - IM.dragDeltaY)
				IM.InventoryManagerFrame:SetPoint("TOPLEFT",UIFrame,"BOTTOMLEFT",IM.mouseX - IM.dragDeltaX,IM.mouseY - IM.dragDeltaY + IM.displayHeight)
				IM.InventoryManagerFrame:SetPoint("BOTTOMRIGHT",UIFrame,"BOTTOMLEFT",IM.mouseX+IM.displayWidth- IM.dragDeltaX,IM.mouseY - IM.dragDeltaY)
				IM.InventoryManagerFrame:SetPoint("TOPRIGHT",UIFrame,"BOTTOMLEFT",IM.mouseX+IM.displayWidth - IM.dragDeltaX,IM.mouseY - IM.dragDeltaY + IM.displayHeight)

				db.isSnapped = false
				
				if IM.InventoryManagerFrame:SnapTo(ContainerFrame1, "Blizzard") then return end
				if IM.InventoryManagerFrame:SnapTo(ZygorGuidesViewerFrame, "Zygor Guide") then return end
	...
	if db.isSnapped then
		...
		elseif db.lastParentName=="Zygor Guide" then
			...
			lastPosition[2] = ZygorGuidesViewerFrame
			db.lastParent = ZygorGuidesViewerFrame
	...
	if db.lastParentName == "Zygor Guide" then
		IM:SetHooks(ZygorGuidesViewerFrame)
	end
	...
	if IM.useAlternateHooks then
		...
		oldCloseBackpack = CloseBackpack
		...
		oldToggleAllBags = ToggleAllBags
	...
	IM.ResizeButton = CHAIN(CreateFrame("Button","ResizeButton",InventoryManagerFrame))
	...
	IM.ConfigButton = CHAIN(CreateFrame("Button","ConfigButton",InventoryManagerFrame))
		...
		:SetScript("OnClick",function() ZygorGuidesViewer:OpenOptions("gold") end)
		:Show()
		...
		:SetScript("OnClick",function(self)
			...
			if not self.Popup then
				...
				self.Popup.returnMinimizeSettings = function(self)
					...
					return notifcationText,L["notifcenter_inventory_manager_title"],tooltipText,priority,poptime,removetime,quiet,onShow
				end
			end
			
			local destroyString = "The following items will be destroyed:\n"
			
			itemList = IM:getGrayItems()
...
function IM:IsItemSoulbound(bagID, bagSlotID)
	local link = GetContainerItemLink(bagID,bagSlotID)
...
local function OnEvent(self, event)
	...
	--if event=="BAG_UPDATE_DELAYED" or event == "QUEST_LOG_UPDATE" or event=="ITEM_PUSH" or event=="PLAYERBANKSLOTS_CHANGED" then --ZGV.db.profile.im_always_wait
	...
	elseif event=="MERCHANT_SHOW" then -- Clear manual waypoints and resume last route.
	...
	elseif event=="ZYGORGV_STEP_CHANGED" then
		--print("ZYGORGV_STEP_CHANGED")
		IM:BagFullCheck(event)
...
function IM:GetBarColor()
	...
	if barFraction < 0.5 then
		return ZGV.CurrentSkinStyle:SkinData("ProgressBarColor") or {0,1,0,1}
...
function IM:UpdateBar()
	...
	if barFraction < 0.5 then
		IM.InventoryMeterBarFrame.tex:SetVertexColor(unpack(ZGV.CurrentSkinStyle:SkinData("ProgressBarColor") or {0,1,0,1}))
...
function IM:RegisterEvents()
	...
	ZGV:AddMessage("ZYGORGV_STEP_CHANGED",OnEvent)
...
function IM:SetHooks(parent, parentName)
	...
	if oldHooks.oldParent then
		oldHooks.oldParent.Hide = oldHide
		oldHooks.oldParent.Show = oldShow
...
do -- limit scope of variables used for threading
	...
	local function GetDistanceToNearestVendorFunction(point)
		...
		for id,data in ZGV.NPCData:iterate(typ) do
			...
			-- create a character.
			local stat, err = pcall(coroutine.status) -- basically a way to detect if we're in a coroutine.
			if count%300==0 and err==nil then
				coroutine.yield(); 
			end
		end
		if minid then
	...
	function IM:FindRecommendedSpaceInBags(returns)
		...
		-- Start the thread
		success, errorText = coroutine.resume(IM.IterateStepsThread, returns)
