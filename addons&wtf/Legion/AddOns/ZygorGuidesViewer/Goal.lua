...
GOALTYPES['learnpet'] = {
	...
	iscomplete = function(self)
		local species = self.petspeciesid or self.parentStep.parentGuide.petSpeciesID or ZGV.PetBattle.SpeciesByDisplayId[self.petid] -- use |petspecies 200 when there is one given, or the guide's header "pet 200" otherwise.
		if not species then return false,true end
...
GOALTYPES['skillmax'] = {
	parse = function(self,params)
		...
		local validlevels={[75]=1,[150]=1,[225]=1,[300]=1,[375]=1,[450]=1,[525]=1,[600]=1,[700]=1,[800]=1}
...
GOALTYPES['goto'] = {
	...
	iscomplete = function(self)
		...
		elseif ZGV.CurrentMapID and not ((ZGV.CurrentMapID==gm or ZGV.CurrentMapID==gm2 or ZGV.CurrentMapID==gm3) and ZGV.CurrentMapFloor==gf) then
...
GOALTYPES['poiaccess'] = {
	parse = function(self,params,step)
		step.poiaccess = params or "Quick"
		if step.poiaccess == "Q" then step.poiaccess = "Quick" end
		if step.poiaccess == "C" then step.poiaccess = "Completionist" end
		self.poiaccess = step.poiaccess 
	end,
	gettext = function(self)
		if ZGV.db.profile.debug_poi then return "poiaccess "..self.poiaccess else return "" end
	end,
}
...
GOALTYPES['poilvl'] = {
	...
	gettext = function(self)
		if ZGV.db.profile.debug_poi then return "poilvl "..self.poilvl else return "" end
	end,
}

GOALTYPES['poispot'] = {
	parse = function(self,params,step)
		local m,_,f,x,y = params:match("([^/]+)(/*)(%d*) (%d.+),(%d.+)")
		step.starts_m = ZGV.LibRover.data.MapIDsByName[m]
		if type(step.starts_m)=="table" then step.starts_m=step.starts_m[1] end -- phased map uglyness
		step.starts_f = tonumber(f)
		step.starts_x = tonumber(x)/100
		step.starts_y = tonumber(y)/100

		--[[
		-- ok, forget about poispots, just store start coordinates in step
		step.poispot = params
		step.poispot_m = ZGV.LibRover.data.MapIDsByName[m]
		if type(step.poispot_m)=="table" then step.poispot_m=step.poispot_m[1] end -- phased map uglyness
		step.poispot_f = tonumber(f)
		step.poispot_x = tonumber(x)/100
		step.poispot_y = tonumber(y)/100
		self.poispot = params
		--]]
	end,
	gettext = function(self)
...
GOALTYPES['poitype'] = {
	...
	gettext = function(self)
		if ZGV.db.profile.debug_poi then return "poitype "..self.poitype else return "" end
	end,
}

GOALTYPES['poiquest'] = {
	parse = function(self,params,step)
		step.poiquest = tonumber(params)
		self.poiquest = tonumber(params)
	end,
	gettext = function(self)
		if ZGV.db.profile.debug_poi then return "poiquest "..self.poiquest else return "" end
	end,
}

GOALTYPES['poiachievement'] = {
	parse = function(self,params,step)
		local _,achieveid,achievesub = ParseID(params)
		if not achievesub and params:find("/") then achieveid = nil end -- They tried to make a sub but did something wrong.
		if not achieveid  then
			return "achieveid and achievesub need to be numbers. format |achieve ###/###" 
		end
		self.poiachieve = tonumber(achieveid)
		self.poisubachieve = tonumber(achievesub)
		self.poiachievename = select(2,GetAchievementInfo(self.poiachieve or 0))
		if self.poisubachieve then
			self.poisubachievename = (Zygor_GetAchievementCriteriaInfo(self.poiachieve,self.poisubachieve))
			step.poisubachievename = self.poisubachievename 
		end
		step.poiachieve = self.poiachieve 
		step.poisubachieve = self.poisubachieve
		step.poiachievename = self.poiachievename
	end,
	gettext = function(self)
		if ZGV.db.profile.debug_poi then return "poiachievement "..self.poiachieve..(self.poisubachieve and "/"..self.poisubachieve or "") else return "" end
	end,
}

GOALTYPES['poipetlevel'] = {
	parse = function(self,params,step)
		step.poipetlevel = params
		self.poipetlevel = params
	end,
	gettext = function(self)
		if ZGV.db.profile.debug_poi then return "poipetlevel "..self.poipetlevel else return "" end
	end,
}

GOALTYPES['poipet'] = {
	parse = function(self,params,step)
		step.poipet = tonumber(params)
		self.poipet = tonumber(params)
	end,
	gettext = function(self)
		if ZGV.db.profile.debug_poi then return "poipet "..self.poipet else return "" end
	end,
}


GOALTYPES['poicurrency'] = {
	parse = function(self,params,step)
		step.poicurrencydata = {}

		for currencystring in string.gmatch(params, "([^,]+)") do
		...
		self.poicurrencydata = step.poicurrencydata 
		self.params = params
	end,
	gettext = function(self)
		if ZGV.db.profile.debug_poi then 
			local returnstring = "Currency: "
			for _,currency in pairs(self.poicurrencydata) do
				returnstring = returnstring..currency.type.." |T"..currency.icon..":0|t, "
			end
			return returnstring:sub(1, -3)
		else return "" end
	end,
}
...
GOALTYPES['poiitem'] = {
	parse = function(self,params,step)
		...
		step.poiitemdata = step.poiitemdata or {}
		local value,item = params:match("([0-9-~]+) (%w+)")
		if not item then  -- just id given
			value,item = nil,params 
		end
		table.insert(step.poiitemdata,{value=value, item=item})

		if item~="RANDOM" then self.itemid = item end
		self.item = item
		self.itemcount = value

		step.poiitemid  = params
	end,
	gettext = function(self)
		if ZGV.db.profile.debug_poi then 
			if self.item == "RANDOM" then
				_,_,_,color = GetItemQualityColor(2)
				return "Reward: "..((self.itemcount and self.itemcount.." ") or "")..("|c"..(color or "ffffffff")).."Random Green"
			else
				local name, link, _, _, _, _, _, _, _, icon = ZGV:GetItemInfo(self.itemid)
				if link then
					return "Reward: "..((self.itemcount and self.itemcount.." ") or "")..link
				else
					return ""
				end
			end
...
GOALTYPES['poiname'] = {
	...
	gettext = function(self)
		if self.parentStep.poitype == "treasure" then 
			return "Treasure: "..self.parentStep.poiname 
		elseif self.parentStep.poitype == "rare" then
			return "Rare: "..self.parentStep.poiname 
		elseif self.parentStep.poitype == "battlepet" then
	...
	onclick = function(self)
		-- reset current active poi number, so that it refocuses on first poi goto lines
		ZGV.Poi.ActivePoiStepNum = -1
	end,
	--[[ Future: Multipoi support
	onclick = function(self)
		local PoiStep = ZGV.Poi.Guide.steps[self.parentStep.num]

		PoiStep.is_expanded = not PoiStep.is_expanded
		PoiStep.flashGoals = PoiStep.is_expanded
		for index,pstep in pairs(ZGV.Poi.Guide.steps) do
			if pstep.num~=self.parentStep.num then
				pstep.is_expanded = false
			end
		end
		ZGV:UpdateFrame()
	end,
	gettooltip = function(self)
		local PoiStep = ZGV.Poi.Guide.steps[self.parentStep.num]
		if PoiStep.is_expanded then
			return L['poi_announce_collapse']
		else
			return L['poi_announce_expand']
		end
	end,
	--]]
}
...
GOALTYPES['poivendor'] = {
	parse = function(self,params,step)
		self.npc,self.npcid = ParseID(params)
		if not self.npc and not self.npcid then print("oops") return end
		self.npc = ZGV.Localizers:GetTranslatedNPC(tonumber(self.npcid))
		step.npc,step.npcid = self.npc,self.npcid
	end,
	gettext = function(self)
		if ZGV.db.profile.debug_poi then return "poivendor "..self.npc.." "..self.npcid else return "" end
	end,
}

GOALTYPES['poicomment'] = {
	parse = function(self,params,step)
		step.poicomment = params
		self.poicomment = params
	end,
	gettext = function(self)
		if ZGV.db.profile.debug_poi then return "poicomment "..self.poicomment else return "" end
	end,
}
...
function Goal:GetText(showcompleteness,brief,showtotals,nocolor)
	...
	if showcompleteness then
		if self.scenario_criteriaid then -- give prio to scenario based counts over quest ones
			local gcnow,gcneeded,gcremain,_,errormsg = GetScenarioGoalData(self.scenario_criteriaid,self.count,self.scenario_stagenum) 
